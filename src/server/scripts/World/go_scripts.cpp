...
class go_soulwell : public GameObjectScript
{
    public:
		...
		bool OnGossipHello(Player* player, GameObject* go) override
		{
			...
			{
				uint32 _stoneSpell = 0;
				uint32 _stoneId = 0;
				switch (go->GetEntry())
				{
					case GO_SOUL_WELL_R1:
						if (owner->ToPlayer()->HasAura(SPELL_IMPROVED_HEALTH_STONE_R1))
							_stoneSpell = SPELL_CREATE_MASTER_HEALTH_STONE_R1;
						else if (owner->ToPlayer()->HasAura(SPELL_IMPROVED_HEALTH_STONE_R2))
...
class go_uldum_pedestal : public GameObjectScript
{
public:
    go_uldum_pedestal() : GameObjectScript("go_uldum_pedestal") { }

    bool OnGossipHello(Player* player, GameObject* go) override
    {
		player->PrepareQuestMenu(go->GetGUID());

        QuestStatus status_a = player->GetQuestStatus(2946);
		QuestStatus status_h = player->GetQuestStatus(2966);
		Creature* pedestal = player->FindNearestCreature(7918, 5.0f, true);
        if (status_a == QUEST_STATUS_REWARDED || status_h == QUEST_STATUS_REWARDED)
	        if (!pedestal)
				player->SummonCreature(7918, -9619.0f, -2815.0f, 11.21f, 0.43f, TEMPSUMMON_TIMED_DESPAWN, 60000);

        return false;
    }
};

class go_om_point : public GameObjectScript
{
public:
    go_om_point() : GameObjectScript("go_om_point") { }
    ...
    bool OnGossipHello(Player* player, GameObject* go) override
    {
		...
		if (!player->IsAlive() || mEntry->IsBattlegroundOrArena() || mEntry->IsDungeon() || mEntry->IsRaid() || player->IsInCombat() || player->HasInvisibilityAura() || player->HasStealthAura() || player->InBattleground() || player->InArena() || zone == 4197)
		{
			player->GetSession()->SendNotification("사용할수없습니다.");
			return true;
		}
		else
		{
			player->TeleportTo(1, -4744.27f, -3659.65f, 49.627f, 1.328935f);
			return true;
		}
    }
};
...
class Second_Chances : public GameObjectScript
{
    public:
        Second_Chances() : GameObjectScript("Second_Chances") { }

        struct Second_ChancesAI : public GameObjectAI
        {
            Second_ChancesAI(GameObject* go) : GameObjectAI(go), _pulseTimer(300000) { }
        
            void UpdateAI(uint32 diff) override
            {
				if (SummonCreature)
				{
					go->CastSpell(nullptr, 18280, true);
        ...
        GameObjectAI* GetAI(GameObject* go) const override
        {
            return new Second_ChancesAI(go);
        }
};
