...
public:
    ...
    struct boss_ahuneAI : public BossAI
    {
        ...
        {
            if (Creature* ahuneBunny = ObjectAccessor::GetCreature(*me, instance->GetData64(DATA_AHUNE_BUNNY)))
        ...
        void JustDied(Unit* /*killer*/) override
        {
            ...
            if (Creature* ahuneBunny = ObjectAccessor::GetCreature(*me, instance->GetData64(DATA_AHUNE_BUNNY)))
                me->Kill(ahuneBunny);
            if (Creature* frozenCore = ObjectAccessor::GetCreature(*me, instance->GetData64(DATA_FROZEN_CORE)))
                me->Kill(frozenCore);
...
public:
    ...
    struct npc_frozen_coreAI : public ScriptedAI
    {
        ...
        void JustDied(Unit* /*killer*/) override
        {
            if (Creature* ahune = ObjectAccessor::GetCreature(*me, _instance->GetData64(DATA_AHUNE)))
                me->Kill(ahune);
        ...
        void DoAction(int32 action) override
        {
            if (action == ACTION_START_EVENT)
            {
                ...
                for (uint8 counter = 0; counter < MAX_FLAMECALLERS; ++counter)
                    if (Creature* flameCaller = ObjectAccessor::GetCreature(*me, _instance->GetData64(DATA_FLAMECALLER_000 + counter)))
            ...
            while (uint32 eventId = _events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_CLOSE_OPENING:
                        if (Creature* flamecaller = ObjectAccessor::GetCreature(*me, _instance->GetData64(DATA_FLAMECALLER_000)))
...
public:
    ...
    struct npc_earthen_ring_flamecallerAI : public ScriptedAI
    {
        ...
        void LookOpening(bool activate, uint8 spot)
        {
            ...
            if (Creature* bonfireBunny = ObjectAccessor::GetCreature(*me, _instance->GetData64(DATA_BONFIRE_BUNNY_000 + spot)))
                if (Creature* beamBunny = ObjectAccessor::GetCreature(*me, _instance->GetData64(DATA_BEAM_BUNNY_000 + spot)))
                {
                    if (activate)
                    {
...
public:
    ...
    bool OnGossipSelect(Player* player, GameObject* go, uint32 /*sender*/, uint32 /*action*/)
    {
		...
		if (instance->GetBossState(DATA_AHUNE) != DONE)
		{
			if (Creature* ahuneBunny = ObjectAccessor::GetCreature(*go, instance->GetData64(DATA_AHUNE_BUNNY)))
			{
				ahuneBunny->AI()->DoAction(ACTION_START_EVENT);
				ahuneBunny->SetInCombatWithZone();
			}

			if (Creature* luma = ObjectAccessor::GetCreature(*go, instance->GetData64(DATA_LUMA_SKYMOTHER)))
...
public:
    ...
    class spell_ice_spear_target_picker_SpellScript : public SpellScript
    {
        ...
        void HandleDummy(SpellEffIndex /*effIndex*/)
        {
            if (Unit* caster = GetCaster())
				if (Unit* target = GetHitUnit())
					caster->CastSpell(target, SPELL_SUMMON_ICE_SPEAR_BUNNY, true);
        }
