    ...
    public:
        ...
        class spell_hun_cobra_strikes_AuraScript : public AuraScript
        {
            ...
            void HandleProc(AuraEffect const* aurEff, ProcEventInfo& /*eventInfo*/)
            {
                ...
                SpellInfo const* triggeredSpellInfo = sSpellMgr->EnsureSpellInfo(GetSpellInfo()->Effects[aurEff->GetEffIndex()].TriggerSpell);
                    ...
                    if (Player* caster = GetCaster()->ToPlayer())
                        if (Pet* target = caster->GetPet())
                        {
							if (target->IsAlive())
							{
								...
								target->CastSpell(caster, GetEffectValue(), castMask);
								target->CastSpell(caster, GetSpellInfo()->Effects[EFFECT_0].CalcValue(), castMask);
								...
								target->CastSpell(ally, GetSpellInfo()->Effects[EFFECT_0].CalcValue(), castMask); // apply 4s root & snare immunity to pet's target
							}
                        }
            }
...
public:
    ...
    class spell_hun_piercing_shots_AuraScript : public AuraScript
    {
        ...
        void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
        {
            ...
            SpellInfo const* piercingShots = sSpellMgr->EnsureSpellInfo(SPELL_HUNTER_PIERCING_SHOTS);
            int32 duration = piercingShots->GetMaxDuration();
            uint32 amplitude = piercingShots->Effects[EFFECT_0].Amplitude;

            uint32 bp = CalculatePct(int32(eventInfo.GetDamageInfo()->GetDamage()), aurEff->GetAmount()) / (duration / int32(amplitude));
    ...
    public:
        ...
        class spell_hun_roar_of_sacrifice_AuraScript : public AuraScript
        {
            ...
            bool CheckProc(ProcEventInfo& eventInfo)
            {
                ...
                if (!damageInfo || !(damageInfo->GetSchoolMask() & (GetEffect(EFFECT_1)->GetMiscValue()) != 0))
                    return false;
...
//Äð´Ù¿îÃß°¡ ¾óÀ½È­»ì, ¾óÀ½ÀÇ µ£, Æø¹ßÀÇ µ£, Á¦¹°ÀÇ µ£ 60210 14311 14310 1499
class spell_hun_spelladdcooldown : public SpellScriptLoader
{
    public:
        spell_hun_spelladdcooldown() : SpellScriptLoader("spell_hun_spelladdcooldown") { }

        class spell_hun_spelladdcooldown_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_hun_spelladdcooldown_SpellScript);
            ...
            SpellCastResult CheckRequirement()
            {
				...
				if (Player* player = caster->ToPlayer())
				{
					player->AddSpellCooldown(60210,0,time(NULL)+24);
					player->AddSpellCooldown(14311,0,time(NULL)+24);
					player->AddSpellCooldown(14310,0,time(NULL)+24);
					player->AddSpellCooldown(1499,0,time(NULL)+24);
				}
                return SPELL_CAST_OK;
            }
            ...
            void Register() override
            {
                OnCheckCast += SpellCheckCastFn(spell_hun_spelladdcooldown_SpellScript::CheckRequirement);
            }
        };

        SpellScript* GetSpellScript() const override
        {
            return new spell_hun_spelladdcooldown_SpellScript();
        }
};

class spell_hun_no_friendly_operation : public SpellScriptLoader
{
    public:
        spell_hun_no_friendly_operation() : SpellScriptLoader("spell_hun_no_friendly_operation") { }

        class spell_hun_no_friendly_operation_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_hun_no_friendly_operation_SpellScript);
            ...
            void Register() override
            {
                OnCheckCast += SpellCheckCastFn(spell_hun_no_friendly_operation_SpellScript::CheckRequirement);
            }
        };

        SpellScript* GetSpellScript() const override
        {
            return new spell_hun_no_friendly_operation_SpellScript();
        }
};
...
void AddSC_hunter_spell_scripts()
{
	...
	new spell_hun_spelladdcooldown();
	new spell_hun_no_friendly_operation();
}
