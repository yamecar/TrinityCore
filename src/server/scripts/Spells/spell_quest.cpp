...
enum Q13264BurstAtTheSeams
{
    NPC_ICY_GHOUL_KC_BUNNY                 = 31142,
	NPC_VICIOUS_GEIST_KC_BUNNY				= 31147,
	NPC_RISEN_SKELETON_KC_BUNNY				= 31205,

    QUEST_BURST_AT_THE_SEAMS1                = 13264,
	QUEST_BURST_AT_THE_SEAMS2                = 13276,
	QUEST_BURST_AT_THE_SEAMS3                = 13288,
	QUEST_BURST_AT_THE_SEAMS4                = 13289,

    SPELL_Q13264_BURST_AT_THE_SEAMS                = 59576, // Burst at the Seams
    SPELL_Q13264_BURST_AT_THE_SEAMS_DMG            = 52508, // Damage spell
    SPELL_Q13264_BURST_AT_THE_SEAMS_DMG_2          = 59580, // Abomination self damage spell
    SPELL_Q13264_BURST_AT_THE_SEAMS_BONE           = 52516, // Burst at the Seams:Bone
    SPELL_Q13264_BURST_AT_THE_SEAMS_MEAT           = 52520, // Explode Abomination:Meat
    SPELL_Q13264_BURST_AT_THE_SEAMS_BMEAT          = 52523, // Explode Abomination:Bloody Meat
};

class spell_q13264_burst_at_the_seams : public SpellScriptLoader
{
    public:
        spell_q13264_burst_at_the_seams() : SpellScriptLoader("spell_q13264_burst_at_the_seams") { }

        class spell_q13264_burst_at_the_seams_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_q13264_burst_at_the_seams_SpellScript);

            bool Validate(SpellInfo const* /*spellInfo*/) override
            {
                if (!sSpellMgr->GetSpellInfo(SPELL_Q13264_BURST_AT_THE_SEAMS)
                    || !sSpellMgr->GetSpellInfo(SPELL_Q13264_BURST_AT_THE_SEAMS_DMG)
                    || !sSpellMgr->GetSpellInfo(SPELL_Q13264_BURST_AT_THE_SEAMS_DMG_2)
                    || !sSpellMgr->GetSpellInfo(SPELL_Q13264_BURST_AT_THE_SEAMS_BONE)
                    || !sSpellMgr->GetSpellInfo(SPELL_Q13264_BURST_AT_THE_SEAMS_MEAT)
                    || !sSpellMgr->GetSpellInfo(SPELL_Q13264_BURST_AT_THE_SEAMS_BMEAT))
                    return false;
                return true;
            }
            ...
            void HandleKnockBack(SpellEffIndex /*effIndex*/)
            {
                if (Unit* creature = GetHitCreature())
                {
                    if (Unit* charmer = GetCaster()->GetCharmerOrOwner())
                    {
                        if (Player* player = charmer->ToPlayer())
                        {
                            if (player->GetQuestStatus(QUEST_BURST_AT_THE_SEAMS1) == QUEST_STATUS_INCOMPLETE || player->GetQuestStatus(QUEST_BURST_AT_THE_SEAMS2) == QUEST_STATUS_INCOMPLETE
								 || player->GetQuestStatus(QUEST_BURST_AT_THE_SEAMS3) == QUEST_STATUS_INCOMPLETE  || player->GetQuestStatus(QUEST_BURST_AT_THE_SEAMS4) == QUEST_STATUS_INCOMPLETE)
                            {
                                creature->CastSpell(creature, SPELL_Q13264_BURST_AT_THE_SEAMS_BONE, true);
                                creature->CastSpell(creature, SPELL_Q13264_BURST_AT_THE_SEAMS_MEAT, true);
                                creature->CastSpell(creature, SPELL_Q13264_BURST_AT_THE_SEAMS_BMEAT, true);
                                creature->CastSpell(creature, SPELL_Q13264_BURST_AT_THE_SEAMS_DMG, true);
                                creature->CastSpell(creature, SPELL_Q13264_BURST_AT_THE_SEAMS_DMG_2, true);

								if (Creature* target = GetHitCreature())
								{
									switch (target->GetEntry())
									{
										case NPC_ICY_GHOUL_KC_BUNNY:
											player->KilledMonsterCredit(31743);
											break;
										case NPC_VICIOUS_GEIST_KC_BUNNY:
											player->KilledMonsterCredit(32168);
											break;
										case NPC_RISEN_SKELETON_KC_BUNNY:
											player->KilledMonsterCredit(32167);
											break;
									}
								}
                            }
            ...
            void Register() override
            {
                OnEffectHitTarget += SpellEffectFn(spell_q13264_burst_at_the_seams_SpellScript::HandleKnockBack, EFFECT_1, SPELL_EFFECT_KNOCK_BACK);
                OnEffectHitTarget += SpellEffectFn(spell_q13264_burst_at_the_seams_SpellScript::HandleScript, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
            }
        };

        SpellScript* GetSpellScript() const override
        {
            return new spell_q13264_burst_at_the_seams_SpellScript();
        }
};
...
void AddSC_quest_spell_scripts()
{
	...
	new spell_q13264_burst_at_the_seams();
}
