        ...
        public:
            ...
            void HandleMount(SpellEffIndex effIndex)
            {
                ...
                if (Player* target = GetHitPlayer())
                {
                    ...
                    if (GetSpellInfo()->Id == 55173) //위습 겨울손아귀호수 비행
					{
						if (!area || (canFly && (area->flags & AREA_FLAG_NO_FLY_ZONE)))
                        canFly = false;
					}
					else
						if (!area || (canFly && (area->flags & AREA_FLAG_NO_FLY_ZONE)) || target->GetZoneId() == 4197)
						canFly = false;
...
class hammer_became_worn_out_storm : public SpellScriptLoader
{
    public:
        hammer_became_worn_out_storm() : SpellScriptLoader("hammer_became_worn_out_storm") { }

        class hammer_became_worn_out_storm_SpellScript : public SpellScript
        {
            PrepareSpellScript(hammer_became_worn_out_storm_SpellScript);
            ...
            SpellCastResult CheckRequirement()
            {
				if (Unit* target = GetExplTargetUnit())
				{
					...
					if (target->GetTypeId() == TYPEID_UNIT)
						if (target->ToCreature()->GetEntry() != 30142)
							return SPELL_FAILED_BAD_TARGETS;
				}
                return SPELL_CAST_OK;
            }
            ...
            void Register() override
            {
                OnCheckCast += SpellCheckCastFn(hammer_became_worn_out_storm_SpellScript::CheckRequirement);
            }
        };

        SpellScript* GetSpellScript() const override
        {
            return new hammer_became_worn_out_storm_SpellScript();
        }
};

//Nitro Boosts
class spell_nitro_boosts : public SpellScriptLoader
{
    public:
        spell_nitro_boosts() : SpellScriptLoader("spell_nitro_boosts") { }

        class spell_nitro_boosts_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_nitro_boosts_SpellScript);

            SpellCastResult CheckRequirement()
            {
				if (Unit* caster = GetCaster())
					if (Player* player = caster->ToPlayer())
						if (player->GetSkillValue(202) < 399)
						{	
							player->GetSession()->SendNotification("기계공학 숙련이 부족합니다.");
							return SPELL_FAILED_CUSTOM_ERROR;
						}
            ...
            void Register() override
            {
                OnCheckCast += SpellCheckCastFn(spell_nitro_boosts_SpellScript::CheckRequirement);
            }
        };

        SpellScript* GetSpellScript() const override
        {
            return new spell_nitro_boosts_SpellScript();
        }
};

class spell_cobalt_frag_bomb : public SpellScriptLoader
{
    public:
        spell_cobalt_frag_bomb() : SpellScriptLoader("spell_cobalt_frag_bomb") { }

        class spell_cobalt_frag_bomb_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_cobalt_frag_bomb_SpellScript);

            SpellCastResult CheckRequirement()
            {
				if (Unit* caster = GetCaster())
					if (Player* player = caster->ToPlayer())
						if (player->GetSkillValue(202) < 349)
						{	
							player->GetSession()->SendNotification("기계공학 숙련이 부족합니다.");
							return SPELL_FAILED_CUSTOM_ERROR;
						}
            ...
            void Register() override
            {
                OnCheckCast += SpellCheckCastFn(spell_cobalt_frag_bomb_SpellScript::CheckRequirement);
            }
        };

        SpellScript* GetSpellScript() const override
        {
            return new spell_cobalt_frag_bomb_SpellScript();
        }
};

//잠행의 대가 삭제
class spell_conversion_characteristics_one : public SpellScriptLoader
{
    public:
        spell_conversion_characteristics_one() : SpellScriptLoader("spell_conversion_characteristics_one") { }

        class spell_conversion_characteristics_one_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_conversion_characteristics_one_SpellScript);
            ...
            SpellCastResult CheckRequirement()
            {
				if (Unit* caster = GetCaster())
					if (Player* player = caster->ToPlayer())
					{
						player->RemoveAurasDueToSpell(31665);
						player->RemoveAurasDueToSpell(31666);
					}
            ...
            void Register() override
            {
                OnCheckCast += SpellCheckCastFn(spell_conversion_characteristics_one_SpellScript::CheckRequirement);
            }
        };

        SpellScript* GetSpellScript() const override
        {
            return new spell_conversion_characteristics_one_SpellScript();
        }
};
...
//그림자 밟기
class stepping_on_the_shadow : public SpellScriptLoader
{
    public:
        stepping_on_the_shadow() : SpellScriptLoader("stepping_on_the_shadow") { }

        class stepping_on_the_shadow_SpellScript : public SpellScript
        {
            PrepareSpellScript(stepping_on_the_shadow_SpellScript);

            SpellCastResult CheckRequirement()
            {
				if (Unit* target = GetExplTargetUnit())
					if (target->GetTypeId() == TYPEID_UNIT)
						if (target->ToCreature()->GetEntry() == 28366)
							return SPELL_FAILED_BAD_TARGETS;
                return SPELL_CAST_OK;
            }
            ...
            void Register() override
            {
                OnCheckCast += SpellCheckCastFn(stepping_on_the_shadow_SpellScript::CheckRequirement);
            }
        };

        SpellScript* GetSpellScript() const override
        {
            return new stepping_on_the_shadow_SpellScript();
        }
};

//야성의 돌진
class feral_Rushstepping_on_the_shadow : public SpellScriptLoader
{
    public:
        feral_Rushstepping_on_the_shadow() : SpellScriptLoader("feral_Rushstepping_on_the_shadow") { }

        class feral_Rushstepping_on_the_shadow_SpellScript : public SpellScript
        {
            PrepareSpellScript(feral_Rushstepping_on_the_shadow_SpellScript);
            ...
            void Register() override
            {
                OnCheckCast += SpellCheckCastFn(feral_Rushstepping_on_the_shadow_SpellScript::CheckRequirement);
            }
        };

        SpellScript* GetSpellScript() const override
        {
            return new feral_Rushstepping_on_the_shadow_SpellScript();
        }
};

//수호자시리즈
class protector_series : public SpellScriptLoader
{
    public:
        protector_series() : SpellScriptLoader("protector_series") { }

        class protector_series_SpellScript : public SpellScript
        {
            PrepareSpellScript(protector_series_SpellScript);
            ...
            void Register() override
            {
                OnCheckCast += SpellCheckCastFn(protector_series_SpellScript::CheckRequirement);
            }
        };

        SpellScript* GetSpellScript() const override
        {
            return new protector_series_SpellScript();
        }
};

class spell_gen_safirdrangs_chill : public SpellScriptLoader
{
    public:
        spell_gen_safirdrangs_chill() : SpellScriptLoader("spell_gen_safirdrangs_chill") { }

        class spell_gen_safirdrangs_chill_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_gen_safirdrangs_chill_SpellScript);
            ...
            void Register() override
            {
                OnCheckCast += SpellCheckCastFn(spell_gen_safirdrangs_chill_SpellScript::CheckCast);
            }
        };

        SpellScript* GetSpellScript() const override
        {
            return new spell_gen_safirdrangs_chill_SpellScript();
        }
};
...
void AddSC_generic_spell_scripts()
{
	...
	new hammer_became_worn_out_storm();
	new spell_nitro_boosts();
	new spell_cobalt_frag_bomb();
	new spell_conversion_characteristics_one();
	new spell_gen_clear_debuffs();

	//new spell_delete_threat_list();  //쓰레드딜리트

	new stepping_on_the_shadow();
	new feral_Rushstepping_on_the_shadow();

	new protector_series(); //수호자시리즈
	new spell_gen_safirdrangs_chill();
}
