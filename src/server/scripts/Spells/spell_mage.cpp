...
/*class spell_delete_threat_list : public SpellScriptLoader
{
    public:
        spell_delete_threat_list() : SpellScriptLoader("spell_delete_threat_list") { }

        class spell_delete_threat_list_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_delete_threat_list_SpellScript);
			...
			//	float myThreat = target->getThreatManager().getThreat(caster);

				target->getThreatManager().doAddThreat(caster, 0 - myThreat);
                return SPELL_CAST_OK;
            }

            void Register() override
            {
                OnCheckCast += SpellCheckCastFn(spell_delete_threat_list_SpellScript::CheckRequirement);
            }
        };

        SpellScript* GetSpellScript() const override
        {
            return new spell_delete_threat_list_SpellScript();
        }
};
*/
