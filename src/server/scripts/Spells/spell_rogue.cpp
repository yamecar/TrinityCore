...
public:
    ...
    class spell_rog_honor_among_thieves_AuraScript : public AuraScript
    {
        ...
        bool CheckProc(ProcEventInfo& /*eventInfo*/)
        {
            ...
            if (!target->HasSpellCooldown(GetSpellInfo()->Effects[EFFECT_0].TriggerSpell))
                return true;
...
public:
    ...
    class spell_rog_honor_among_thieves_proc_SpellScript : public SpellScript
    {
        ...
        {
            Unit* target = GetHitUnit();
			Player* target_add = GetCaster()->ToPlayer();
            if (!target)
                return;
            ...
            target_add->AddSpellCooldown(GetSpellInfo()->Id, 0, time(NULL) + 1);
        }
