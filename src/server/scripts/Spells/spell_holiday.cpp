...
public:
    ...
    class spell_midsummer_braziers_hit_AuraScript : public AuraScript
    {
        ...
        void HandleEffectApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            ...
            if (player->GetAura(SPELL_TORCH_TOSSING_TRAINING) && GetStackAmount() == 8 || player->GetAura(SPELL_TORCH_TOSSING_PRACTICE) && GetStackAmount() == 20)
            {
        ...
        void Register() override
        {
            AfterEffectApply += AuraEffectApplyFn(spell_midsummer_braziers_hit_AuraScript::HandleEffectApply, EFFECT_0, SPELL_AURA_DUMMY, AuraEffectHandleModes(AURA_EFFECT_HANDLE_REAL | AURA_EFFECT_HANDLE_REAPPLY));
        }
    };
        ...
        class spell_gen_juggle_torch_catch_SpellScript : public SpellScript
        {
            ...
            void FilterTargets(std::list<WorldObject*>& unitList)
            {
                ...
                if (missed)
                {
                    if (juggleTarget)
					{	
						Unit* teg = juggleTarget->ToUnit();
                        teg->CastSpell(teg, SPELL_JUGGLE_TORCH_MISSED, true);
					}
