...
const char* Transmogrification::GetSlotName(uint8 slot, WorldSession* /*session*/) const
{
    ...
    switch (slot)
    {
    case EQUIPMENT_SLOT_HEAD: return  "머리";// session->GetTrinityString(LANG_SLOT_NAME_HEAD);
    case EQUIPMENT_SLOT_SHOULDERS: return  "어깨";// session->GetTrinityString(LANG_SLOT_NAME_SHOULDERS);
    case EQUIPMENT_SLOT_BODY: return  "셔츠";// session->GetTrinityString(LANG_SLOT_NAME_BODY);
    case EQUIPMENT_SLOT_CHEST: return  "가슴";// session->GetTrinityString(LANG_SLOT_NAME_CHEST);
    case EQUIPMENT_SLOT_WAIST: return  "허리";// session->GetTrinityString(LANG_SLOT_NAME_WAIST);
    case EQUIPMENT_SLOT_LEGS: return  "다리";// session->GetTrinityString(LANG_SLOT_NAME_LEGS);
    case EQUIPMENT_SLOT_FEET: return  "발";// session->GetTrinityString(LANG_SLOT_NAME_FEET);
    case EQUIPMENT_SLOT_WRISTS: return  "손목";// session->GetTrinityString(LANG_SLOT_NAME_WRISTS);
    case EQUIPMENT_SLOT_HANDS: return  "손";// session->GetTrinityString(LANG_SLOT_NAME_HANDS);
    case EQUIPMENT_SLOT_BACK: return  "등";// session->GetTrinityString(LANG_SLOT_NAME_BACK);
    case EQUIPMENT_SLOT_MAINHAND: return  "주장비";// session->GetTrinityString(LANG_SLOT_NAME_MAINHAND);
    case EQUIPMENT_SLOT_OFFHAND: return  "보조장비";// session->GetTrinityString(LANG_SLOT_NAME_OFFHAND);
    case EQUIPMENT_SLOT_RANGED: return  "원거리무기";// session->GetTrinityString(LANG_SLOT_NAME_RANGED);
    case EQUIPMENT_SLOT_TABARD: return  "겉옷";// session->GetTrinityString(LANG_SLOT_NAME_TABARD);
    default: return NULL;
    }
}
...
void Transmogrification::DeleteFakeEntry(Player* player, Item* item)
{
	...
	QueryResult result = CharacterDatabase.PQuery("DELETE FROM `custom_transmogrification` WHERE `Owner` = %u and `GUID` = %u",player->GetGUIDLow(),item->GetGUID());
}
...
bool Transmogrification::CanTransmogrifyItemWithItem(Player* player, ItemTemplate const* target, ItemTemplate const* source) const
{
    ...
    {
		if (target->Class == ITEM_CLASS_ARMOR && target->SubClass == ITEM_SUBCLASS_ARMOR_SHIELD)
			if (source->InventoryType == INVTYPE_HOLDABLE || source->InventoryType == INVTYPE_RELIC)
				return true;

		if (source->Class == ITEM_CLASS_ARMOR && source->SubClass == ITEM_SUBCLASS_ARMOR_SHIELD)
			return true;

		if (Item* item = player->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_MAINHAND))
			if (item->GetEntry() == target->ItemId)
				if (source->Class == ITEM_CLASS_WEAPON && (IsRangedWeapon(target->Class, target->SubClass) != IsRangedWeapon(source->Class, source->SubClass) || source->InventoryType == INVTYPE_WEAPONOFFHAND ))
					return false;

		if (Item* item = player->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_OFFHAND))
			if (item->GetEntry() == target->ItemId)
				if (source->Class == ITEM_CLASS_WEAPON && (IsRangedWeapon(target->Class, target->SubClass) != IsRangedWeapon(source->Class, source->SubClass) || source->InventoryType == INVTYPE_WEAPONMAINHAND ))
					return false;
        ...
        void OnLogin(Player* player, bool /*firstLogin*/) override
        {
            ...
            if (result)
            {
                do
                {
					...
					QueryResult result_check = CharacterDatabase.PQuery("SELECT * FROM item_instance WHERE `itemEntry` = %u and `owner_guid` = %u", fakeEntry, player->GetGUIDLow());
					...
					//else
						//CharacterDatabase.PExecute("DELETE FROM custom_transmogrification WHERE FakeEntry = %u and Owner = %u", fakeEntry, player->GetGUIDLow());
