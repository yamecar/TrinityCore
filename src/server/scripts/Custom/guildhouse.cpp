...
void SellGuildHouse(Player * pPlayer, Creature * pCreature)
{
	if(isInGuildArea(pPlayer))
	{
		...
		pCreature->MonsterSay("길드하우스가 판매 되었습니다!", 0, 0);
	...
	{
		pPlayer->GetSession()->SendNotification("판매할수있는 길드하우스에 있어야 합니다!");
	}
}
...
void PurchaseGuildHouse(Player * pPlayer, Creature * pCreature, uint32 &Id)
{
	if(pPlayer->HasItemCount(GUILD_TOKEN, 1))
	{
		...
		pCreature->MonsterSay("축하합니다 새로운 길드하우스를 구입하셨습니다!", 0, 0);
		pPlayer->DestroyItemCount(GUILD_TOKEN, 1, true);
		requiresGUpdate = true;
	}
	else
	{
		pCreature->MonsterSay("길드하우스를 구입할수있는 토근이 필요합니다!", 0, 0);
	}
		pPlayer->PlayerTalkClass->SendCloseGossip();
}
...
class GuildHouse_NPC : public CreatureScript
{
	public:
		...
		void ShowHouseListings(Player * pPlayer, Creature * pCreature, unsigned int start = 0)
		{
			...
			for(unsigned int i = start; i < houses; i++)
			{
				...
				else
				{
					...
					{
						pPlayer->ADD_GOSSIP_ITEM(7, "-------------끝--------------", GOSSIP_SENDER_MAIN, 41);
					}
					else
					{
						pPlayer->ADD_GOSSIP_ITEM(7, "다음페이지", GOSSIP_SENDER_MAIN, 4353);
		...
		bool OnGossipHello(Player * pPlayer, Creature * pCreature)
		{
			...
			{
				pPlayer->GetSession()->SendNotification("길드에 가입되어 있어야 합니다!");
				return false;
			}
			...
			pPlayer->ADD_GOSSIP_ITEM(4, "길드 하우스:", GOSSIP_SENDER_MAIN, 41);
			pPlayer->ADD_GOSSIP_ITEM(4, "----------------------------------------------", GOSSIP_SENDER_MAIN, 41);
			if(isPlayerGuildLeader(pPlayer))
			{
				if(!GuildOwnsHouse(pPlayer))
					pPlayer->ADD_GOSSIP_ITEM(6, "길드 하우스 구입", GOSSIP_SENDER_MAIN, 38);
				else
					pPlayer->ADD_GOSSIP_ITEM_EXTENDED(6, "길드 하우스 판매", GOSSIP_SENDER_MAIN, 39, "정말로 판매를 하시겠습니까?", 0, false);
			}
			if(GuildOwnsHouse(pPlayer))
				pPlayer->ADD_GOSSIP_ITEM(7, "길드 하우스로 이동", GOSSIP_SENDER_MAIN, 40);
			pPlayer->PlayerTalkClass->SendGossipMenu(2, pCreature->GetGUID());
			return true;
		}
...
class Table_Token : public ItemScript
{
	public:
		...
		bool OnUse(Player* pPlayer, Item* pItem, SpellCastTargets const& /*targets*/)
		{
			...
			{
				pPlayer->GetSession()->SendNotification("길드하우스 소유자여야 합니다!");
				return false;
			}

			if(!isInGuildArea(pPlayer))
			{
				pPlayer->GetSession()->SendNotification("길드하우스에 있지않습니다!");
				return false;
			}
...
class Torch_Token : public ItemScript
{
	public:
		...
		bool OnUse(Player* pPlayer, Item* pItem, SpellCastTargets const& /*targets*/)
		{
			...
			if(!isInGuildArea(pPlayer))
			{
				pPlayer->GetSession()->SendNotification("길드하우스에 있지 않습니다!");
				return false;
			}
...
class Chair_Token : public ItemScript
{
	public:
		...
		bool OnUse(Player* pPlayer, Item* pItem, SpellCastTargets const& /*targets*/)
		{
			...
			if(!isInGuildArea(pPlayer))
			{
				pPlayer->GetSession()->SendNotification("길드하우스 있지않습니다!");
				return false;
			}
...
class Guild_Token_Vendor : public CreatureScript
{
	public:
		...
		bool OnGossipHello(Player * pPlayer, Creature * pCreature)
		{
			if(!isPlayerGuildLeader(pPlayer))
			{
				pPlayer->GetSession()->SendNotification("길드마스만 권한이 있습니다!");
				pPlayer->PlayerTalkClass->SendCloseGossip();
				return false;
			}
			...
			{
				pPlayer->PlayerTalkClass->ClearMenus();
				pPlayer->ADD_GOSSIP_ITEM(4, "토근을 구입하고싶습니다!", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_TRADE);
				pPlayer->ADD_GOSSIP_ITEM(3, "괜찮습니다.", GOSSIP_SENDER_MAIN, 2);
				pPlayer->PlayerTalkClass->SendGossipMenu(2, pCreature->GetGUID());
				//return true;
			}
			else
