...
class npc_captain_arnath : public CreatureScript
{
    public:
        ...
        struct npc_captain_arnathAI : public npc_argent_captainAI
        {
            ...
            void UpdateAI(uint32 diff) override
            {
				if (pash && me->getFaction() == 2209)
				{
					me->GetMotionMaster()->MovePoint(631,4356.124f, 2484.33f, 358.489, 4.672945);
...
class npc_deathbound_ward : public CreatureScript
{
    public:
        ...
        struct npc_deathbound_wardAI : public CreatureAI
        {
            npc_deathbound_wardAI(Creature* creature) : CreatureAI(creature)
            {
            }

            void Reset() override
            {
				_disruptingshout = 12000;
				_saberlash = 3000;
            }
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                if (_disruptingshout <= diff)
                {
                    _disruptingshout = 12000;
                     DoCastVictim(71022);
                }
                else
                    _disruptingshout -= diff;
...
class teleport_to_the_upper_spire : public CreatureScript
{
    public:
        teleport_to_the_upper_spire() : CreatureScript("teleport_to_the_upper_spire") { }

        struct teleport_to_the_upper_spireAI : public CreatureAI
        {
            teleport_to_the_upper_spireAI(Creature* creature) : CreatureAI(creature),instance(creature->GetInstanceScript())
            {
            }
			...
			bool IsAnyPlayerInRange()
            {
				...
				if (instance->GetData(DATA_UPPERSPIRE_TELE_ACT) == DONE)
					return false;
                ...
                for (Map::PlayerList::const_iterator itr = playerList.begin(); itr != playerList.end(); ++itr)
                {
                   if (Player* player = itr->GetSource())
                    {
                        if (player->IsAlive() && me->GetDistance(player) < 20.0f)
                            return true;
                    }
                }
            }
        ...
        CreatureAI* GetAI(Creature* creature) const override
        {
            return new teleport_to_the_upper_spireAI(creature);
        }
};
...
void AddSC_icecrown_citadel()
{
	...
	new teleport_to_the_upper_spire();
}
