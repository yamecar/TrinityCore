            ...
            if (InstanceScript* instance = go->GetInstanceScript())
            {
                if (instance->GetBossState(DATA_LORD_MARROWGAR) == DONE)
				{
                    if (go->GetEntry() != GO_SCOURGE_TRANSPORTER_LIGHTSHAMMER)
                        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "빛의 망치 순간이동.", GOSSIP_SENDER_ICC_PORT, LIGHT_S_HAMMER_TELEPORT);
                    if (go->GetEntry() != GO_SCOURGE_TRANSPORTER_ORATORY)
                        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "저주받은 자들의 기도실 순간이동.", GOSSIP_SENDER_ICC_PORT, ORATORY_OF_THE_DAMNED_TELEPORT);
                }
                if (instance->GetBossState(DATA_LADY_DEATHWHISPER) == DONE && go->GetEntry() != GO_SCOURGE_TRANSPORTER_RAMPART)
                    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "해골 망루 순간이동.", GOSSIP_SENDER_ICC_PORT, RAMPART_OF_SKULLS_TELEPORT);

                if (instance->GetBossState(DATA_ICECROWN_GUNSHIP_BATTLE) == DONE && go->GetEntry() != GO_SCOURGE_TRANSPORTER_DEATHBRINGER)
                    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "죽음의 인도자 마루 순간이동.", GOSSIP_SENDER_ICC_PORT, DEATHBRINGER_S_RISE_TELEPORT);

                if (instance->GetData(DATA_UPPERSPIRE_TELE_ACT) == DONE  && go->GetEntry() != GO_SCOURGE_TRANSPORTER_UPPERSPIRE)
                    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "첨탑 상층 순간이동.", GOSSIP_SENDER_ICC_PORT, UPPER_SPIRE_TELEPORT);

                /// @todo Gauntlet event before Sindragosa
                if (instance->GetBossState(DATA_SINDRAGOSA_GAUNTLET) == DONE)
                    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "신드라고사의 둥지 순간이동", GOSSIP_SENDER_ICC_PORT, SINDRAGOSA_S_LAIR_TELEPORT);
            }
