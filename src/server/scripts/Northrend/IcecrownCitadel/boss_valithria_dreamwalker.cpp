        ...
        struct boss_valithria_dreamwalkerAI : public ScriptedAI
        {
            ...
            void HealReceived(Unit* healer, uint32& heal) override
            {
                ...
                else if (!_over75PercentTalkDone && me->HealthAbovePctHealed(75, heal))
                {
					...
					if (Creature* lichKing = ObjectAccessor::GetCreature(*me, _instance->GetData64(DATA_VALITHRIA_LICH_KING)))
                        lichKing->AI()->DoAction(ACTION_HELTH75);
                }
...
*/
            ...
            void UpdateAI(uint32 /*diff*/) override
            {
                ...
                {
                    EnterEvadeMode();
					instance->SetBossState(DATA_VALITHRIA_DREAMWALKER, NOT_STARTED);  //custom
    ...
    public:
        ...
        struct npc_the_lich_king_controllerAI : public ScriptedAI
        {
            ...
            void Reset() override
            {
                ...
                me->SetReactState(REACT_PASSIVE);
				over75Percent = false;
				gluttonous_summoner = 1;
				suppresser_summoner = 1;
				zombie_summoner = 1;
				risen_summoner = 1;
				blazing_summoner = 1;
            }
					...
					if (IsHeroic())
					{
						switch (eventId)
						{
						    case EVENT_GLUTTONOUS_ABOMINATION_SUMMONER:
								...
								if (over75Percent && gluttonous_summoner <= 50)
								{
									_events.ScheduleEvent(EVENT_GLUTTONOUS_ABOMINATION_SUMMONER, 5000 - (gluttonous_summoner * 50));
									++gluttonous_summoner;
								}
								break;
							case EVENT_SUPPRESSER_SUMMONER:
								DoCast(me, SPELL_TIMER_SUPPRESSER);
								if (over75Percent && suppresser_summoner <= 50)
								{
									_events.ScheduleEvent(EVENT_SUPPRESSER_SUMMONER, 10000 - (suppresser_summoner * 100));
									++suppresser_summoner;
								}
								break;
							case EVENT_BLISTERING_ZOMBIE_SUMMONER:
								DoCast(me, SPELL_TIMER_BLISTERING_ZOMBIE);
								if (over75Percent && zombie_summoner <= 50)
								{
									_events.ScheduleEvent(EVENT_BLISTERING_ZOMBIE_SUMMONER, 15000 - (zombie_summoner * 150));
									++zombie_summoner;
								}
								break;
							case EVENT_RISEN_ARCHMAGE_SUMMONER:
								DoCast(me, SPELL_TIMER_RISEN_ARCHMAGE);
								if (over75Percent && risen_summoner <= 50)
								{
									_events.ScheduleEvent(EVENT_RISEN_ARCHMAGE_SUMMONER, 20000 - (risen_summoner * 200));
									++risen_summoner;
								}
								break;
							case EVENT_BLAZING_SKELETON_SUMMONER:
								DoCast(me, SPELL_TIMER_BLAZING_SKELETON);
								if (over75Percent && blazing_summoner <= 50)
								{
									_events.ScheduleEvent(EVENT_BLAZING_SKELETON_SUMMONER, 30000 - (blazing_summoner * 300));
									++blazing_summoner;
								}
								break;
							default:
        ...
        private:
            ...
            InstanceScript* _instance;
			bool over75Percent;
			uint32 gluttonous_summoner;
			uint32 suppresser_summoner;
			uint32 zombie_summoner;
			uint32 risen_summoner;
			uint32 blazing_summoner;
        };
