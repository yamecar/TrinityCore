						...
						for (Map::PlayerList::const_iterator i = lPlayers.begin(); i != lPlayers.end(); ++i)
							if (Player* player = i->GetSource())
							{
								if (Item* item = player->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_MAINHAND))
									if (item->GetEntry() == 49623)
										if (player->GetQuestStatus(24748) == QUEST_STATUS_INCOMPLETE)
										{
											player->AddItem(51315,1);
											player->KilledMonsterCredit(38153);
										}
							}
				}
            }
...
*/
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                {
					if (eventId == EVENT_VILE_SPIRITS)
					{
						me->ApplySpellImmune(0, IMMUNITY_STATE, SPELL_AURA_MOD_TAUNT, true);  //도발면역
						me->ApplySpellImmune(0, IMMUNITY_EFFECT, SPELL_EFFECT_ATTACK_ME, true);
					}
					else
					{
						me->ApplySpellImmune(0, IMMUNITY_STATE, SPELL_AURA_MOD_TAUNT, false);  //도발면역 해제
                    ...
                    switch (eventId)
                    {
                        ...
                        case EVENT_HARVEST_SOULS:
							...
							summons.DespawnEntry(NPC_VILE_SPIRIT);
                            break;
                        case EVENT_FROSTMOURNE_HEROIC:
                            ...
                            {
								Creature* trigger1 = me->SummonCreature(NPC_WORLD_TRIGGER_INFINITE_AOI, 495.7080f, -2523.760f, 1050.000f, 0.0f, TEMPSUMMON_TIMED_DESPAWN, 41000);
								Creature* trigger2 = me->SummonCreature(NPC_WORLD_TRIGGER_INFINITE_AOI, 514.554f, -2523.16f, 1049.95f, 0.0f, TEMPSUMMON_TIMED_DESPAWN, 41000);
                ...
                switch (id)
                {
                    ...
                    case POINT_CHARGE:
                        if (Player* target = ObjectAccessor::GetPlayer(*me, _grabbedPlayer))
                        {
                            ...
                            {
                                std::list<Creature*> triggers;
                                GetCreatureListWithEntryInGrid(triggers, me, NPC_WORLD_TRIGGER, 300.0f);
                ...
                while (uint32 eventId = _events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_FROSTMOURNE_TALK_1:
							...
							_instance->SetData(DATA_NPC_SPIRIT_BOOMB, IN_PROGRESS);
							me->DespawnOrUnsummon(41000);
                            if (IsHeroic())
							{
    ...
    public:
        ...
        private:
            ...
            void HandleScript(SpellEffIndex effIndex)
            {
				...
				GetCaster()->CastSpell(GetHitUnit(), 43807, true); // custom
            }
