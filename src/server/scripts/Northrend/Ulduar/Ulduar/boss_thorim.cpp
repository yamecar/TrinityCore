    ...
    public:
        ...
        struct boss_thorimAI : public BossAI
        {
            ...
            void Reset() override
            {
                ...
                if (Creature* controller = ObjectAccessor::GetCreature(*me, instance->GetData64(DATA_THORIM_CONTROLLER)))
            ...
            void SpellHit(Unit* /*caster*/, SpellInfo const* spellInfo) override
            {
                ...
                {
                    if (Creature* sif = ObjectAccessor::GetCreature(*me, instance->GetData64(DATA_SIF)))
                    {
            ...
            bool CanStartPhase2(Unit* actor) const
            {
                ...
                Creature* runicColossus = ObjectAccessor::GetCreature(*me, instance->GetData64(DATA_RUNIC_COLOSSUS));
                Creature* runeGiant = ObjectAccessor::GetCreature(*me, instance->GetData64(DATA_RUNE_GIANT));
...
struct npc_thorim_trashAI : public ScriptedAI
{
    ...
    struct AIHelper
    {
        ...
        static Unit* GetHealTarget(uint32 spellId, Unit* caster)
        {
            ...
            if (SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(spellId))
            {
                if ((!spellInfo->AttributesEx & SPELL_ATTR1_CANT_TARGET_SELF) && !roll_chance_f(caster->GetHealthPct()) && !((caster->GetHealth() + GetRemainingHealOn(caster) + GetTotalHeal(spellInfo, caster)) > caster->GetMaxHealth()))
                    healTarget = caster;
                else
    ...
    public:
        ...
        struct npc_thorim_arena_phaseAI : public npc_thorim_trashAI
        {
            ...
            void EnterCombat(Unit* /*who*/)
            {
				...
				if (!_isInArena)
                    if (Creature* colossus = ObjectAccessor::GetCreature(*me, _instance->GetData64(DATA_RUNIC_COLOSSUS)))
...
struct npc_thorim_minibossAI : public ScriptedAI
{
	...
	void DoAction(int32 action) override
    {
        ...
        {
            for (uint64 const& guid : _summons)
    ...
    public:
        ...
        struct npc_runic_colossusAI : public npc_thorim_minibossAI
        {
            ...
            void Reset() override
            {
                ...
                // Runed Door closed
                _instance->HandleGameObject(_instance->GetData64(DATA_RUNIC_DOOR), false);
            ...
            {
                // Runed Door opened
                _instance->HandleGameObject(_instance->GetData64(DATA_RUNIC_DOOR), true);

                if (Creature* thorim = ObjectAccessor::GetCreature(*me, _instance->GetData64(BOSS_THORIM)))
                    thorim->AI()->Talk(SAY_SPECIAL_2);

				if (Creature* giant = ObjectAccessor::GetCreature(*me, _instance->GetData64(DATA_RUNE_GIANT)))
                {
    ...
    public:
        ...
        struct npc_ancient_rune_giantAI : public npc_thorim_minibossAI
        {
            ...
            void Reset() override
            {
                ...
                // Stone Door closed
                _instance->HandleGameObject(_instance->GetData64(DATA_STONE_DOOR), false);
            ...
            {
                // Stone Door opened
                _instance->HandleGameObject(_instance->GetData64(DATA_STONE_DOOR), true);
            }
    ...
    public:
        ...
        struct npc_sifAI : public ScriptedAI
        {
            ...
            void SpellHit(Unit* /*caster*/, SpellInfo const* spellInfo) override
            {
                ...
                {
					if (Creature* blizzard = ObjectAccessor::GetCreature(*me, _instance->GetData64(DATA_SIF_BLIZZARD)))
    ...
    public:
        ...
        class spell_thorim_blizzard_SpellScript : public SpellScript
        {
            ...
            void SelectTarget(std::list<WorldObject*>& targets)
            {
                ...
                if (Creature* blizzard = ObjectAccessor::GetCreature(*GetCaster(), _instance->GetData64(DATA_SIF_BLIZZARD)))
        ...
        class spell_thorim_activate_lightning_orb_periodic_AuraScript : public AuraScript
        {
            ...
            void PeriodicTick(AuraEffect const* /*aurEff*/)
            {
                ...
                if (!triggers.empty())
                {
                    ...
                    if (Creature* Thorim = ObjectAccessor::GetCreature(*caster, instance->GetData64(BOSS_THORIM)))
...
class condition_thorim_arena_leap : public ConditionScript
{
    public:
        ...
        bool OnConditionCheck(Condition* condition, ConditionSourceInfo& sourceInfo) override
        {
            ...
            return target->GetGUID() != instance->GetData64(DATA_SIF_BLIZZARD) && _check(target);
        }
