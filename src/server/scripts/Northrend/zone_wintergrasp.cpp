...
#define GOSSIP_HELLO_DEMO1  "겨울손아귀 투석기를 만들어주세요."
#define GOSSIP_HELLO_DEMO2  "겨울손아귀 파괴전차를 만들어주세요."
#define GOSSIP_HELLO_DEMO3  "겨울손아귀 공성 전차를 만들어주세요."
#define GOSSIP_HELLO_DEMO4  "지금은 더 만들수가 없습니다!"
                    ...
                    if (Battlefield* wg = sBattlefieldMgr->GetBattlefieldByBattleId(BATTLEFIELD_BATTLEID_WG))
                       ...
                       for (uint8 i = 0; i < MAX_WINTERGRASP_VEHICLES; i++)
                            if (Creature* vehicleCreature = go->FindNearestCreature(vehiclesList[i], 3.0f, true))
                                if (!vehicleCreature->HasAura(SPELL_VEHICLE_TELEPORT) && vehicleCreature->GetCharmerOrOwnerPlayerOrPlayerItself() && vehicleCreature->GetCharmerOrOwnerPlayerOrPlayerItself()->GetTeamId() == wg->GetDefenderTeam())
                                    if (Creature* teleportTrigger = vehicleCreature->FindNearestCreature(234720, 100.0f, true))
...
class npc_wg_quest_giver : public CreatureScript
{
    public:
		...
		struct npc_wg_quest_giverAI : public CreatureAI
        {
            npc_wg_quest_giverAI(Creature* creature) : CreatureAI(creature)
            {
            }
            ...
            void Reset() override
            {
				...
				Heal = 8000;
				HolyNova = 12000;
				PowerWordShield = 15000;
				HealingWave = 8000;
				LightningBolt = 12000;
				QuickFlameWard = 15000;
            ...
            void UpdateAI(uint32 diff) override
            {
				if (!me->GetVictim() || (me->GetHomePosition().GetPositionX() - me->GetPositionX() >= 45.0f) || (me->GetHomePosition().GetPositionY() - me->GetPositionY() >= 45.0f)
					|| (me->GetPositionX()  - me->GetHomePosition().GetPositionX() >= 45.0f) || (me->GetPositionY() - me->GetHomePosition().GetPositionY() >= 45.0f))
				{
					me->GetMotionMaster()->MoveTargetedHome();
					me->CombatStop();
					me->DeleteThreatList();
				...
				if (me->GetEntry() == 31107 || me->GetEntry() == 31109 || me->GetEntry() == 31051 || me->GetEntry() == 31106 || me->GetEntry() == 31153)
				{
				...
				if (me->GetEntry() == 31091 || me->GetEntry() == 31036 || me->GetEntry() == 31053)
				{
					 if (Cleave <= diff)
					{
						Cleave = 12000;
				...
				if (me->GetEntry() == 31054)
				{
					...
					if (PowerWordShield <= diff)
					{
						PowerWordShield = 15000;
						 DoCastVictim(17139);
					}
					else
						PowerWordShield -= diff;
				}

				if (me->GetEntry() == 31101)
				{
					 if (HealingWave <= diff)
					{
						HealingWave = 8000;
