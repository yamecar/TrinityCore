...
enum Events
{
	...
	//custom
	EVENT_NPC_ORB_CARRIER		= 18,
	EVENT_NPC_LIVING_INFERNO	= 19,
};
    ...
    public:
        ...
        struct boss_halionAI : public BossAI
        {
            ...
            void EnterCombat(Unit* /*who*/) override
            {
                ...
                events.ScheduleEvent(EVENT_BREATH, urand(5000, 15000));
                events.ScheduleEvent(EVENT_CLEAVE, urand(6000, 10000));
                events.ScheduleEvent(EVENT_TAIL_LASH, urand(7000, 12000));
                events.ScheduleEvent(EVENT_FIERY_COMBUSTION, urand(15000, 18000));
                events.ScheduleEvent(EVENT_METEOR_STRIKE, 18000);
				...
				while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_TAIL_LASH:
                            ...
                            events.ScheduleEvent(EVENT_TAIL_LASH, urand(11000, 16000));
                            break;
                        case EVENT_BREATH:
                            DoCast(me, SPELL_FLAME_BREATH);
                            events.ScheduleEvent(EVENT_BREATH, urand(16000, 25000));
                            break;
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_TAIL_LASH:
                            DoCastAOE(SPELL_TAIL_LASH);
                            events.ScheduleEvent(EVENT_TAIL_LASH, urand(12000, 16000));
                            break;
                        case EVENT_BREATH:
                            DoCast(me, SPELL_DARK_BREATH);
                            events.ScheduleEvent(EVENT_BREATH, urand(10000, 14000));
                            break;
...
class npc_halion_controller : public CreatureScript
{
    public:
        ...
        struct npc_halion_controllerAI : public ScriptedAI
        {
            ...
            {
                if (_instance->GetData64(DATA_HALION))
                    return;
            ...
            void DoAction(int32 action) override
            {
                switch (action)
                {
					...
					case ACTION_ACTIVATE_EMBERS:
                        _events.ScheduleEvent(EVENT_ACTIVATE_EMBERS, 6000);
                        break;
                    default:
                        break;
                }
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                if (!_events.IsInPhase(PHASE_INTRO) && me->IsInCombat() && !UpdateVictim())
                {
					...
					{
						EnterEvadeMode();
						_instance->SetBossState(DATA_HALION, FAIL);  //custom
					}
                    return;
                }
        ...
        struct npc_orb_carrierAI : public ScriptedAI
        {
            ...
            void UpdateAI(uint32 diff) override
            {
                ...
                if (!me->HasUnitState(UNIT_STATE_CASTING))
				 ...
				 if (events.ExecuteEvent() == EVENT_NPC_ORB_CARRIER)
            ...
            void DoAction(int32 action) override
            {
                switch (action)
                {
                    case ACTION_WARNING_SHOOT:
                    {
						...
						events.ScheduleEvent(EVENT_NPC_ORB_CARRIER, 5000);
                    }
					case ACTION_SHOOT:
                    {
                        Vehicle* vehicle = me->GetVehicleKit();
        ...
        struct npc_living_infernoAI : public ScriptedAI
        {
            ...
            void IsSummonedBy(Unit* /*summoner*/) override
            {
                ...
                if (me->GetMap()->GetDifficulty() == RAID_DIFFICULTY_25MAN_HEROIC)
                    events.ScheduleEvent(EVENT_NPC_LIVING_INFERNO, 3000);
			...
			void UpdateAI(uint32 diff) override
			{
				...
				if (events.ExecuteEvent() == EVENT_NPC_LIVING_INFERNO)
