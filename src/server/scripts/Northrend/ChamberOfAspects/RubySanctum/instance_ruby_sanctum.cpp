...
class instance_ruby_sanctum : public InstanceMapScript
{
    public:
        ...
        struct instance_ruby_sanctum_InstanceMapScript : public InstanceScript
        {
            ...
            {
                if (!GetData64(DATA_HALION) && GetBossState(DATA_HALION) != DONE && GetBossState(DATA_GENERAL_ZARITHRIAN) == DONE)
                {
                    instance->LoadGrid(HalionControllerSpawnPos.GetPositionX(), HalionControllerSpawnPos.GetPositionY());
                    if (Creature* halionController = instance->GetCreature(GetData64(DATA_HALION_CONTROLLER)))
