...
enum Events
{
	EVENT_SPELL_UTGARDE_PINNACLE_GUANTLET_RESET_CHECK	= 0,
	EVENT_SPELL_RIDE_GRAUF								= 1,

	EVENT_SPELL_CRUSH									= 2,
	EVENT_SPELL_POISONED_SPEAR							= 3,
	EVENT_SPELL_WHIRLWIND								= 4,

	EVENT_GRAUF_PATH_INITIAL							= 5,
	EVENT_POINT_LEFT									= 6,
	EVENT_EMOTE_BREATH									= 7,
	EVENT_SPELL_FREEZING_CLOUD_LEFT_PERIODIC			= 8,
	EVENT_SPELL_FREEZING_CLOUD_LEFT_PERIODIC_REMOVE		= 9,
	EVENT_SPELL_FREEZING_CLOUD_RIGHT_PERIODIC			= 10,
	EVENT_SPELL_FREEZING_CLOUD_RIGHT_PERIODIC_REMOVE	= 11,

	EVENT_POINT_9										= 12,
	EVENT_SPELL_HAMSTRING								= 13,
	EVENT_SPELL_STRIKE									= 14,
	EVENT_SPELL_SHADOW_BOLT								= 15,
	EVENT_SPELL_SHRINK									= 16,
	EVENT_SPELL_NET										= 17,
	EVENT_SPELL_THROW									= 18,

	EVENT_EMOTE_ON_RANGE								= 19
};
...
public:
    ...
    struct boss_skadiAI : public BossAI
    {
        ...
        void Reset() override
        {
            ...
            if (!ObjectAccessor::GetCreature(*me, instance->GetData64(DATA_GRAUF)))
        ...
        void DoAction(int32 action) override
        {
            switch (action)
            {
                case ACTION_START_ENCOUNTER:
					...
					events.ScheduleEvent(EVENT_SPELL_UTGARDE_PINNACLE_GUANTLET_RESET_CHECK, 6000);
					events.ScheduleEvent(EVENT_SPELL_RIDE_GRAUF, 2000);
                ...
                case ACTION_GAUNTLET_END:
					...
					events.ScheduleEvent(EVENT_SPELL_CRUSH, 8000);
					events.ScheduleEvent(EVENT_SPELL_POISONED_SPEAR, 11000);
					events.ScheduleEvent(EVENT_SPELL_WHIRLWIND, 23000);
                    break;
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
				{
					case EVENT_SPELL_UTGARDE_PINNACLE_GUANTLET_RESET_CHECK:
					...
					case EVENT_SPELL_CRUSH:
						...
						events.ScheduleEvent(SPELL_CRUSH, 8000);
						break;
					case EVENT_SPELL_POISONED_SPEAR:
						if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM))
                                DoCast(target, SPELL_POISONED_SPEAR);
                            events.ScheduleEvent(SPELL_POISONED_SPEAR, 11000);
						break;
					case EVENT_SPELL_WHIRLWIND:
						DoCast(SPELL_WHIRLWIND);
                        events.ScheduleEvent(SPELL_WHIRLWIND, 23000);
						break;
					default:
                        break;
				}
    ...
    struct npc_graufAI : public ScriptedAI
    {
        ...
        void PassengerBoarded(Unit* who, int8 /*seatId*/, bool apply) override
        {
          ...
          //  me->SetByteFlag(UNIT_FIELD_BYTES_1, UNIT_BYTES_1_OFFSET_ANIM_TIER, UNIT_BYTE1_FLAG_ALWAYS_STAND | UNIT_BYTE1_FLAG_HOVER);
			...
			events.ScheduleEvent(EVENT_GRAUF_PATH_INITIAL, 2000);
        }
        ...
        void MovementInform(uint32 type, uint32 pointId) override
        {
			...
			switch (pointId)
            {
                case POINT_9:
                    ...
                    // no break
                case POINT_11:
					events.ScheduleEvent(EVENT_EMOTE_ON_RANGE, 1000);
					events.ScheduleEvent(EVENT_POINT_LEFT, 1000);
                    break;
                case POINT_LEFT:
					events.ScheduleEvent(EVENT_EMOTE_BREATH, 1000);
					events.ScheduleEvent(EVENT_SPELL_FREEZING_CLOUD_LEFT_PERIODIC, 2000);
					events.ScheduleEvent(EVENT_SPELL_FREEZING_CLOUD_LEFT_PERIODIC_REMOVE, 10000);
                    break;
                case POINT_RIGHT:
					events.ScheduleEvent(EVENT_EMOTE_BREATH, 1000);
					events.ScheduleEvent(EVENT_SPELL_FREEZING_CLOUD_RIGHT_PERIODIC, 2000);
					events.ScheduleEvent(EVENT_SPELL_FREEZING_CLOUD_RIGHT_PERIODIC_REMOVE, 10000);
                    break;
				default:
	                break;
            }
    ...
    void MovementInform(uint32 type, uint32 pointId) override
    {
        ...
        switch (pointId)
        {
            case POINT_0:
                ...
                break;
            case POINT_1:
				events.ScheduleEvent(EVENT_POINT_9, 1000);
                break;
            case POINT_9:
...
public:
    ...
    struct npc_ymirjar_warriorAI : public npc_skadi_trashAI
    {
		...
		void Reset() override
		{
			events.ScheduleEvent(EVENT_SPELL_HAMSTRING, 2000);
			events.ScheduleEvent(EVENT_SPELL_STRIKE, 9000);
		}
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
				{
					case EVENT_SPELL_HAMSTRING:
						...
						events.ScheduleEvent(EVENT_SPELL_HAMSTRING, urand(11000, 18000));
						break;
					case EVENT_SPELL_STRIKE:
						DoCastVictim(SPELL_STRIKE);
						events.ScheduleEvent(EVENT_SPELL_STRIKE, urand(10000, 13000));
						break;
				}
			}
			DoMeleeAttackIfReady();
...
public:
    ...
    struct npc_ymirjar_witch_doctorAI : public npc_skadi_trashAI
    {
		...
		void Reset() override
		{
			events.ScheduleEvent(EVENT_SPELL_SHADOW_BOLT, 2000);
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
				{
					...
					case EVENT_SPELL_SHRINK:
						...
						events.ScheduleEvent(EVENT_SPELL_SHRINK, urand(20000, 34000));
						break;
				}
			}
			DoMeleeAttackIfReady();
...
public:
    ...
    struct npc_ymirjar_harpoonerAI : public npc_skadi_trashAI
    {
		...
		void Reset() override
		{
			events.ScheduleEvent(EVENT_SPELL_NET, 13000);
			events.ScheduleEvent(EVENT_SPELL_THROW, 2000);
		}
