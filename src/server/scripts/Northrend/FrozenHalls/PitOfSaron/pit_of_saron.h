    ...
    public:
        ...
        bool Execute(uint64 /*time*/, uint32 /*diff*/) override
        {
            if (roll_chance_i(12))
            {
                ...
                _trigger->m_Events.AddEvent(new ScheduledIcicleSummons(_trigger), _trigger->m_Events.CalculateTime(urand(1000, 10000)));
            }
            else
                _trigger->m_Events. AddEvent(new ScheduledIcicleSummons(_trigger), _trigger->m_Events.CalculateTime(urand(1000,5000)));
