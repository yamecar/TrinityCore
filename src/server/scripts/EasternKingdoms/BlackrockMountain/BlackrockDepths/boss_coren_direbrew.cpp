...
enum DirebrewEvents
{
	...
	EVENT_SPELL_MOLE_MACHINE_EMERGE,
	EVENT_GO_MOLE_MACHINE_TRAP,
	EVENT_SPELL_HAS_DARK_BREWMAIDENS_BREW
};
...
public:
    ...
    struct boss_coren_direbrewAI : public BossAI
    {
        ...
        void MoveInLineOfSight(Unit* who) override
        {
            ...
            events.ScheduleEvent(EVENT_INTRO_1, 6000, 0, PHASE_INTRO);
            Talk(SAY_INTRO);
        }
        ...
        void DoAction(int32 action) override
        {
            if (action == ACTION_START_FIGHT)
            {
                ...
                events.ScheduleEvent(EVENT_SUMMON_MOLE_MACHINE, 15000);
                events.ScheduleEvent(EVENT_DIREBREW_DISARM, 20000);
            }
        }
        ...
        void SummonedCreatureDies(Creature* summon, Unit* /*killer*/) override
        {
            if (summon->GetEntry() == NPC_ILSA_DIREBREW)
                events.ScheduleEvent(EVENT_RESPAWN_ILSA, 1000);
            else if (summon->GetEntry() == NPC_URSULA_DIREBREW)
                events.ScheduleEvent(EVENT_RESPAWN_URSULA, 1000);
        }
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_INTRO_1:
                        Talk(SAY_INTRO1);
                        events.ScheduleEvent(EVENT_INTRO_2, 4000, 0, PHASE_INTRO);
                        break;
                    case EVENT_INTRO_2:
                    {
                        EntryCheckPredicate pred(NPC_ANTAGONIST);
                        summons.DoAction(ACTION_ANTAGONIST_SAY_1, pred);
                        events.ScheduleEvent(EVENT_INTRO_3, 3000, 0, PHASE_INTRO);
                        break;
                    }
                    case EVENT_INTRO_3:
                    {
    ...
    struct npc_coren_direbrew_sistersAI : public ScriptedAI
    {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
					case EVENT_SPELL_HAS_DARK_BREWMAIDENS_BREW:
						if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0, 0.0f, false, -SPELL_HAS_DARK_BREWMAIDENS_BREW))
							DoCast(target, SPELL_CHUCK_MUG);
						events.ScheduleEvent(EVENT_SPELL_HAS_DARK_BREWMAIDENS_BREW, 4000);
						break;
				}
			}
...
public:
    ...
    struct npc_direbrew_minionAI : public ScriptedAI
    {
        ...
        {
            if (Creature* coren = ObjectAccessor::GetCreature(*me, _instance->GetData64(DATA_COREN)))
...
public:
    ...
    struct go_direbrew_mole_machineAI : public GameObjectAI
    {
        ...
        void Reset() override
        {
			...
			events.ScheduleEvent(EVENT_SPELL_MOLE_MACHINE_EMERGE, 1000);
			events.ScheduleEvent(EVENT_GO_MOLE_MACHINE_TRAP, 4000);
        }
