...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        if (player->GetQuestStatus(12801) == QUEST_STATUS_INCOMPLETE)
            player->ADD_GOSSIP_ITEM(0, "전 준비되었습니다. 대영주님. 바로 희망의 빛 예배당을 포위 공격합시다!", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);
                ...
                if (uiPhase_timer <= diff)
                {
                    ...
                    switch (uiStep)
                    {
                        ...
                        case 48: // Show the cleansing effect (dawn of light)
                            ...
                            if (GameObject* go = me->SummonGameObject(GO_LIGHT_OF_DAWN, 2283.896f, -5287.914f, 83.066f, 0, 0, 0, 0, 0, 30000))
							{
								uiDawnofLightGUID = go->GetGUID();
								if (Creature* temp = ObjectAccessor::GetCreature(*me, uiTirionGUID))
								{
