/*
 * Copyright (C) 2006-2012 SkCore <http://www.skwow.net/>
 *
...
#define GOSSIP_POSSES "Ich ubernehme!" //need correction
...
#define SAY_LOSE_KNIGHT_M "Ja... naturlich."
...
public:
        ...
        {
                chess_move_triggerAI(Creature* c) : ScriptedAI(creature), _instance(creature->GetInstanceScript())
                {
                }
                    ...
                    if(unit_on_me != NULL)
                        {
                                if(search_time < diff )
                        {
                        if(Creature->GetDistance2d(unit_on_me) != 0 || unit_on_me->isAlive())
                        {
                                unit_on_me = NULL;
                        }
...
void SpellHit(Unit *caster,const SpellEntry *spell)
{
                ...
                || spell->Id == SPELL_MOVE_7)
        {
                ...
                caster->GetMotionMaster()->MovePoint(0,creature->GetPositionX(),creature->GetPositionY(),creature->GetPositionZ());
                unit_on_me = caster;
                DoCast(creature,SPELL_MOVE_MARKER);
        }
}
};
    ...
    struct chess_npcAI : public ScriptedAI
        {
                ...
                {
                        if (creature->getVictim() && who->isTargetableForAttack() && ( creature->IsHostileTo( who )) && who->isInAccessiblePlaceFor(creature) )
                        {
                                if (!creature->canFly() && creature->GetDistanceZ(who) > CREATURE_Z_ATTACK_RANGE)
                                        return;
 
                                if( creature->IsWithinDistInMap(who, NPC_ATTACK_RADIUS) && creature->IsWithinLOSInMap(who) && isInGame)
                                {
                ...
                void Aggro(Unit *Unit)
                {
                        ...
                        npc_medivh = (Creature*)Unit::GetUnit(*creature,ImageGUID);
            ...
            if (player->GetTeam() == ALLIANCE)
                        {
                    if(creature->GetEntry() == NPC_KING_H)
                                {
                            npc_medivh->Say(SAY_MEDIVH_CHECK,LANG_UNIVERSAL,NULL);
                    }
                                else if(creature->GetEntry() == NPC_KING_A)
                                {
                        ...
                        {
                                if(creature->GetEntry() == NPC_KING_A)
                                {
                                        npc_medivh->Say(SAY_MEDIVH_CHECK,LANG_UNIVERSAL,NULL);
                                }
                                else if(creature->GetEntry() == NPC_KING_H)
                                {
                                        npc_medivh->Say(SAY_PLAYER_CHECK,LANG_UNIVERSAL,NULL);
                                }
                        }
        ...
        void OnPossess(bool Apply)
        {
                if(Apply)
                {
                        original_faction_of_player = creature->GetCharmer()->getFaction();
                        creaturecharmer = creature->GetCharmer();
            ...
            if(player->GetTeam() == ALLIANCE)
                        {
                                creature->setFaction(ALLI_CREATURE_FACTION);
                                creaturecharmer->setFaction(ALLI_CREATURE_FACTION);
                        }
                        else
                        {
                                creature->setFaction(HORDE_CREATURE_FACTION);
        ...
        std::list<Unit*> FindPossibleMoveUnits()
        {
                ...
                CellPair p(Trinity::ComputeCellPair(creature->GetPositionX(), creature->GetPositionY()));
        Cell cell(p);
        ...
        float range = GetStrafeLenght(creature->GetEntry())*SEARCH_RANGE;
 
        Trinity::AnyUnitInObjectRangeCheck u_check(creature,range);
        ...
        cell_lock->Visit(cell_lock, grid_unit_searcher, *(creature->GetMap()));
        ...
        void UpdateAI(const uint32 diff)
        {
                ...
                if(!creature->isPossessed())
                {
                        ...
                        if(creature->getVictim() || !creature->IsWithinDistInMap(creature->getVictim(), NPC_ATTACK_RADIUS))
                        {
                                creature->CombatStop();
                                return;
                        }
