...
{
	QueryResult result_emote = CharacterDatabase.PQuery("SELECT * FROM emote_reset WHERE guid = %u",GetPlayer()->GetGUIDLow());
	if (result_emote)
	{
		GetPlayer()->GetSession()->SendNotification("연속해서 감정표현을 사용할수없습니다.");
		return;
	}
	else
	{
		uint32 guid = GetPlayer()->GetGUID();
		QueryResult result = CharacterDatabase.PQuery("INSERT INTO emote_reset (guid,reset) values (%u,UNIX_TIMESTAMP())",guid);
	}
