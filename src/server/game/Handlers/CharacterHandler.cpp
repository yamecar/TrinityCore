...
void WorldSession::HandlePlayerLogin(LoginQueryHolder* holder)
{
	...
	{	
		if (bg)
		{
			pCurrChar->LeaveBattleground();
			bg->EventPlayerLoggedOut(pCurrChar);
	...
	if (pCurrChar->GetSession()->IsPremium())
			SendNotification("현재 5배율 경험치로 탐험하고 있습니다.");

	if (!pCurrChar->IsGameMaster())
	{
		if (pCurrChar->getLevel() >= 10)
		{
			QueryResult result = CharacterDatabase.PQuery("SELECT * FROM auth.vote_log WHERE recruiterid = '%d AND activate = 0'", GetAccountId());
			if(result)
			{
				QueryResult result2 = CharacterDatabase.PQuery("UPDATE auth.vote_log set activate = 1 WHERE recruiterid = '%d'",GetAccountId());
			}
		}

		QueryResult result_ban = CharacterDatabase.PQuery("SELECT * FROM daily_players_reports WHERE account = '%d' and activ = 0",GetAccountId());
		if (result_ban)
		{
			uint32 accId = GetAccountId();
			...
			QueryResult result_update = CharacterDatabase.PQuery("UPDATE daily_players_reports set activ = 1 WHERE guid = %d",pCurrChar->GetGUIDLow());
			KickPlayer();  
		}

		//잉여ip차단
		QueryResult result = CharacterDatabase.PQuery("SELECT * FROM character_ip_banned WHERE ip = '%s'", IP_str.c_str());
		if(result)
		{
			KickPlayer();
			return;
		...
		//듀얼아이피차단
		QueryResult result2 = CharacterDatabase.PQuery("SELECT Count(last_ip) FROM auth.account WHERE last_ip = '%s' and online = 1", IP_str.c_str());
		if(result2)
		{
			uint32 counte = (*result2)[0].GetUInt32();
			uint32 ch_count = 2;

			QueryResult result4 = CharacterDatabase.PQuery("SELECT * FROM auth.character_ip_dupl WHERE account = '%d'", GetAccountId());
			if(result4)
				QueryResult result5 = CharacterDatabase.PQuery("UPDATE auth.character_ip_dupl SET ip = '%s' WHERE account = '%d'", IP_str.c_str(),GetAccountId());

			QueryResult result3 = CharacterDatabase.PQuery("SELECT * FROM auth.character_ip_dupl WHERE ip = '%s'", IP_str.c_str());
			if(result3)
				ch_count = (*result3)[1].GetUInt32();
		...
		QueryResult result3 = CharacterDatabase.PQuery("SELECT * FROM auth.account WHERE last_ip = '%s'", IP_str.c_str());
		if (result3)
		{
			do
			{
				uint32 oldaccount = (*result3)[0].GetUInt32();
				//race=종족 (1: 인간, 2: 오크, 3: 드웝, 4: 나엘, 5: 언데, 6: 소, 7:노움, 8:트롤, 10:블러드엘프,11:드레나이)
				if (pCurrChar->getRace() == 1 || pCurrChar->getRace() == 3 || pCurrChar->getRace() == 4 || pCurrChar->getRace() == 7 || pCurrChar->getRace() == 11 )
				{
					QueryResult result4 = CharacterDatabase.PQuery("SELECT * FROM characters WHERE account = '%d' and race IN (2,5,6,8,10)",oldaccount);
					if (result4)
						KickPlayer();
				}

				if (pCurrChar->getRace() == 2 || pCurrChar->getRace() == 5 || pCurrChar->getRace() == 6 || pCurrChar->getRace() == 8 || pCurrChar->getRace() == 10 )
				{
					QueryResult result4 = CharacterDatabase.PQuery("SELECT * FROM characters WHERE account = '%d' and race IN (1,3,4,7,11)",oldaccount);
					if (result4)
						KickPlayer();
				} 
			}
	...
	/*	if (pCurrChar->getRace() == 2 || pCurrChar->getRace() == 5 || pCurrChar->getRace() == 6 || pCurrChar->getRace() == 8 || pCurrChar->getRace() == 10 )
			if (pCurrChar->GetGuildId() == 0)
				if (GetAccountId() != 2333 || GetAccountId() != 2604 || GetAccountId() != 6017)
				{
					uint64 targetGuid = pCurrChar->GetGUIDLow();
					std::string guildName = "자리 비움";
