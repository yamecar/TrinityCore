...
void WorldSession::HandleBattleFieldPortOpcode(WorldPacket &recvData)
{
    ...
    {
		if (_player->HasAura(11593)) //전장신정시간
			_player->RemoveAurasDueToSpell(11593);

		uint32 guid = _player->GetGUID();
		QueryResult result_del = CharacterDatabase.PQuery("DELETE FROM battle_join_reset WHERE guid = %u",guid);
    ...
    else // leave queue 대기취소
    {
		...
		uint32 arena_team_id = _player->GetArenaTeamId(0);
		QueryResult result_del__join = CharacterDatabase.PQuery("DELETE FROM arena_dual_join WHERE TeamId = '%u'",arena_team_id);
    }
}
...
void WorldSession::HandleBattlemasterJoinArena(WorldPacket& recvData)
{
	...
	if (arenaslot == 2)
	{
		_player->GetSession()->SendNotification("현재 5V5 투기장 신청이 불가능합니다.");
		return;
	}

	if (arenaslot == 1)
	{
		_player->GetSession()->SendNotification("현재 3V3 투기장 신청이 불가능합니다.");
		return;
	}
...
/*	if (asGroup)
	{
		if (t->tm_hour < 18)
			if (t->tm_hour > 2)
			{
				_player->GetSession()->SendNotification("현재 투기장 진행시간이 아닙니다. 운영시간 18:00 ~ 익일 02:00");
				return;
			}

		grp = _player->GetGroup();
		uint32 leader_guid = _player->GetGUID();
		std::string playerIp = _player->GetSession()->GetRemoteAddress();
		uint32 arena_team_id = _player->GetArenaTeamId(0);

		QueryResult result_check = CharacterDatabase.PQuery("SELECT * FROM arena_dual_join WHERE playerip = '%s'",playerIp.c_str());
		if (result_check)
		{
			_player->GetSession()->SendNotification("단일 아이피로 1계정만 신청할수있습니다.");
			return;
		}
		...
		{
			Player* member = itr->GetSource();
			uint32 member_guid = member->GetGUID();
			if (!member)
				continue;

			std::string memberIp = member->GetSession()->GetRemoteAddress();

			if (leader_guid != member_guid)
			{
				if (playerIp == memberIp)
				{
					_player->GetSession()->SendNotification("단일아이피 팀은 신청할수없습니다.");
					return;
				}

				QueryResult result_sc_check = CharacterDatabase.PQuery("SELECT * FROM arena_dual_join WHERE playerip = '%s'",memberIp.c_str());
				if (result_sc_check)
				{
					_player->GetSession()->SendNotification("단일 아이피로 1계정만 신청할수있습니다.");
					return;
		...
		QueryResult result = CharacterDatabase.PQuery("REPLACE INTO arena_dual_join (TeamId,playerip,time) values (%u,'%s',UNIX_TIMESTAMP())",arena_team_id,playerIp.c_str());
	} */
	//투기장 이중접속차단 종료
