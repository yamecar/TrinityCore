...
void ObjectAccessor::applicantAllPlayers(uint32 team, uint64 guid)
{
	Player* applicantplayer = ObjectAccessor::FindPlayer(guid);
    TRINITY_READ_GUARD(HashMapHolder<Player>::LockType, *HashMapHolder<Player>::GetLock());
    HashMapHolder<Player>::MapType const& m = GetPlayers();
    for (HashMapHolder<Player>::MapType::const_iterator itr = m.begin(); itr != m.end(); ++itr)
	{
		if (Player* player = itr->second)
			if (player->GetTeam() == team)
				ChatHandler(player->GetSession()).PSendSysMessage("|cffff0000[전장]:|r 신청자 %s ", applicantplayer->GetName().c_str());
	}
}

//보스킬
void ObjectAccessor::bosskillreword(uint64 guid)
{
	if (Unit* Unit = ObjectAccessor::FindUnit(guid))
	{
		Creature* Creature = Unit->ToCreature();
		Map::PlayerList const& lPlayers = Creature->GetMap()->GetPlayers();
		if (!lPlayers.isEmpty())
			for (Map::PlayerList::const_iterator i = lPlayers.begin(); i != lPlayers.end(); ++i)
			{
				if ((Creature->GetMap()->IsDungeon()) && (Creature->GetMap()->IsHeroic()) && (Creature->GetMap()->GetPlayersCountExceptGMs() >= 4))
					if (Player* player = i->GetSource())
						player->AddItem(7168,1);

				if (Creature->GetMap()->IsRaid() && (Creature->GetMapId() != 631 || Creature->GetMapId() != 724))
				{
					if (Creature->GetMap()->Is25ManRaid())
					{
						if (Creature->GetMap()->GetPlayersCountExceptGMs() >= 15)
							if (Player* player = i->GetSource())
								player->AddItem(7168,3);
					}
					else
					{
						if (Creature->GetMap()->GetPlayersCountExceptGMs() >= 7)
							if (Player* player = i->GetSource())
								player->AddItem(7168,2);
					}
				}
			}
	}
...
void ObjectAccessor::RemoveAllPlayersALLIANCEBuffer()
{
    TRINITY_READ_GUARD(HashMapHolder<Player>::LockType, *HashMapHolder<Player>::GetLock());
    HashMapHolder<Player>::MapType const& m = GetPlayers();
    for (HashMapHolder<Player>::MapType::const_iterator itr = m.begin(); itr != m.end(); ++itr)
	{
		if (Player* player = itr->second)
			if (player->GetTeam() == ALLIANCE)
				player->RemoveAurasDueToSpell(18209);
	}
}

void ObjectAccessor::RemoveAllPlayersHORDEBuffer()
{
    TRINITY_READ_GUARD(HashMapHolder<Player>::LockType, *HashMapHolder<Player>::GetLock());
    HashMapHolder<Player>::MapType const& m = GetPlayers();
    for (HashMapHolder<Player>::MapType::const_iterator itr = m.begin(); itr != m.end(); ++itr)
