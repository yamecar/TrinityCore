...
void Guardian::UpdateDamagePhysical(WeaponAttackType attType)
{
	...
	if(GetEntry() == ENTRY_VOIDWALKER || GetEntry() == ENTRY_SUCCUBUS || GetEntry() == ENTRY_FELHUNTER ||GetEntry() == ENTRY_FELGUARD)
	{
		if (m_owner->GetAuraEffect(18769, 0)) 
		{
            mindamage = (((base_value + weapon_mindamage) * base_pct + total_value) * total_pct) * 1.04f;
            maxdamage = (((base_value + weapon_maxdamage) * base_pct + total_value) * total_pct) * 1.04f;
		}
		else if (m_owner->GetAuraEffect(18770, 0))
		{
            mindamage = (((base_value + weapon_mindamage) * base_pct + total_value) * total_pct) * 1.08f;
            maxdamage = (((base_value + weapon_maxdamage) * base_pct + total_value) * total_pct) * 1.08f;
		}
		else if (m_owner->GetAuraEffect(18771, 0))
		{
            mindamage = (((base_value + weapon_mindamage) * base_pct + total_value) * total_pct) * 1.12f;
            maxdamage = (((base_value + weapon_maxdamage) * base_pct + total_value) * total_pct) * 1.12f;
		}
		else if (m_owner->GetAuraEffect(18772, 0))
		{
            mindamage = (((base_value + weapon_mindamage) * base_pct + total_value) * total_pct) * 1.16f;
            maxdamage = (((base_value + weapon_maxdamage) * base_pct + total_value) * total_pct) * 1.16f;
		}
		else if (m_owner->GetAuraEffect(18773, 0))
		{
            mindamage = (((base_value + weapon_mindamage) * base_pct + total_value) * total_pct) * 1.20f;
            maxdamage = (((base_value + weapon_maxdamage) * base_pct + total_value) * total_pct) * 1.20f;
		}
    }
