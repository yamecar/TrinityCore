...
uint32 Unit::DealDamage(Unit* victim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellInfo const* spellProto, bool durabilityLoss)
{
	...
	uint32 accId = ToPlayer()->GetSession()->GetAccountId();	// get account id
	if (GetTypeId() == TYPEID_PLAYER && accId != 1 )
	{
		uint32 check_damage = 120000;
		if (victim->GetTypeId() == TYPEID_PLAYER && this != victim)
			check_damage = 120000;

		if (damage >= check_damage)  // 9만데미지 이상 디비기록
		{
			uint32 guid = this->ToPlayer()->GetGUIDLow();
			uint32 entry = 0;
			uint32 type = 1;
			uint32 damage_type = 0;

			if (spellProto)
			{
				if (spellProto->Id == 33836 || spellProto->Id == 56578 || spellProto->Id == 66588 || spellProto->Id == 64591 || spellProto->Id == 59234 || spellProto->Id == 20625 || spellProto->Id == 35139 || spellProto->Id == 53209 || spellProto->Id == 49143 || spellProto->Id == 56448 || spellProto->Id == 23449)
					return 0;

				damage_type = spellProto->Id;
			}

			if (victim->GetTypeId() == TYPEID_PLAYER && this != victim)
			{
				entry = victim->ToPlayer()->GetGUIDLow();
				type = 0;
			}

			if (victim->GetTypeId() == TYPEID_UNIT && this != victim)
			{
				entry = victim->ToCreature()->GetEntry();
				type = 1;
			}

			QueryResult result_insert = CharacterDatabase.PQuery("insert INTO damage_hack (guid,entry,damage,time,type,damage_type) values ('%d','%d','%d',UNIX_TIMESTAMP(),'%d','%d')",guid,entry,damage,type,damage_type); 
			damage = 0;
		}
	}
...
void Unit::CastStop(uint32 except_spellid)
{
    for (uint32 i = CURRENT_FIRST_NON_MELEE_SPELL; i < CURRENT_MAX_SPELL; i++)
        if (m_currentSpells[i] && m_currentSpells[i]->m_spellInfo->Id != except_spellid)
            InterruptSpell(CurrentSpellTypes(i), false); //케스팅 취소
}
...
void Unit::CalculateSpellDamageTaken(SpellNonMeleeDamage* damageInfo, int32 damage, SpellInfo const* spellInfo, WeaponAttackType attackType, bool crit)
{
	...
	if (spellInfo->Id == 71433 || spellInfo->Id == 71434)  //단지 안에 든 작은 누더기골렘
		damage += damage * 0.35f;
...
{
	if (victim->HasAura(70911) || victim->HasAura(72854) || victim->HasAura(72855) || victim->HasAura(72856))  //그림자망토 고삐풀린역병
		if (spellInfo->Id == 31224)
			return SPELL_MISS_RESIST;
...
bool Unit::CanMoveDuringChannel() const
{
    if (Spell* spell = m_currentSpells[CURRENT_CHANNELED_SPELL])
        if (spell->getState() != SPELL_STATE_FINISHED)
            return (spell->GetSpellInfo()->AttributesEx5 & SPELL_ATTR5_CAN_CHANNEL_WHEN_MOVING) && spell->IsChannelActive();
...
void Unit::GetDispellableAuraList(Unit* caster, uint32 dispelMask, DispelChargesList& dispelList)
{
    ...
    for (AuraMap::const_iterator itr = auras.begin(); itr != auras.end(); ++itr)
    {
		...
		//독살 디스펠리스트 제거
		if (aura->GetSpellInfo()->SpellIconID == 2237)
			continue;
...
// Used in case when access to whole aura is needed 확률발동
...
bool Unit::HandleAuraProc(Unit* victim, uint32 damage, Aura* triggeredByAura, SpellInfo const* procSpell, uint32 /*procFlag*/, uint32 procEx, uint32 cooldown, bool * handled)
{
    ...
    switch (dummySpell->SpellFamilyName)
    {
            ...
            switch (dummySpell->Id)
            {
                ...
                case 72833:
                {
                    *handled = true;
                    ...
                    int32 const mod = (GetMap()->GetSpawnMode() & 1) ? 3000 : 2500;
                    int32 dmg = 0;
		...
		case SPELLFAMILY_HUNTER:
		{
            if (victim && (victim->HasAura(3674) || victim->HasAura(63668) || victim->HasAura(63669) || victim->HasAura(63670) || victim->HasAura(63671) || victim->HasAura(63672) || victim->HasAura(49064) || victim->HasAura(49065)))
			{
				if (HasAura(56342))
				{
					if (!roll_chance_i(2))
...
/*	if (auraSpellInfo->SpellIconID == 2237)
        if (auraSpellInfo->SpellIconID == 1673)
            return false;
*/
...
void Unit::UpdateSpeed(UnitMoveType mtype, bool forced)
{
    ...
    /*    float baseSpeed = (GetTypeId() == TYPEID_UNIT ? ToCreature()->GetCreatureTemplate()->speed_walk : 1.0f);
        ...
        if (!IS_PLAYER_GUID(GetOwnerGUID()) && !IsHunterPet() && GetTypeId() == TYPEID_UNIT)
...
void Unit::ModSpellCastOrDurationTime(SpellInfo const* spellInfo, int32 & castTime, Spell* spell, bool modDuration)
{
    if (!spellInfo || castTime < 0)
        return;

	if (modDuration && spellInfo->IsChanneled() && !(spellInfo->AttributesEx5 & SPELL_ATTR5_HASTE_AFFECT_DURATION))
        return;
...
void Unit::ProcDamageAndSpellFor(bool isVictim, Unit* target, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, SpellInfo const* procSpell, uint32 damage, SpellInfo const* procAura)
{
    ...
    for (AuraApplicationMap::const_iterator itr = GetAppliedAuras().begin(); itr!= GetAppliedAuras().end(); ++itr)
    {
        ...
        // Defensive procs are active on absorbs (so absorption effects are not a hindrance) 데미지 흡수 발동
        bool active = false;
        if (isVictim)
		{
