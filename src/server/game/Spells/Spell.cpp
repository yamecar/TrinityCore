...
void Spell::DoAllEffectOnTarget(TargetInfo* target)
{
    ...
    if ((m_spellInfo->AttributesEx7 & SPELL_ATTR7_INTERRUPT_ONLY_NONPLAYER) && unit->GetTypeId() != TYPEID_PLAYER)
...
SpellMissInfo Spell::DoSpellHitOnUnit(Unit* unit, uint32 effectMask, bool scaleAura)
{
    if (!unit || !effectMask)
	 ...
	 if (Player* player = unit->ToPlayer())  //칼날폭풍 회오리바람면역
		if (player->HasAura(46924))
			if (m_spellInfo->SpellIconID == 174)
				return SPELL_MISS_IMMUNE;

	if (unit->GetTypeId() == TYPEID_UNIT && unit->ToCreature()->GetEntry() == 34775) //고대의 해안 전차 회오리바람 면역
		if (m_spellInfo->SpellIconID == 174)
			return SPELL_MISS_IMMUNE;

	if (Player* player = unit->ToPlayer())  //얼음 방패
		if (player->HasAura(45438))
			if (!m_spellInfo->SpellIconID == 2267 || !m_spellInfo->SpellIconID == 3998 || !m_spellInfo->SpellIconID == 3182)
				return SPELL_MISS_RESIST;
	
	if (Player* player = unit->ToPlayer())  //독살 정화
		if (player->HasAura(32645) || player->HasAura(32684) || player->HasAura(57992) || player->HasAura(57993))
			if (m_spellInfo->Id == 52025)
				return SPELL_MISS_RESIST;
        ...
        if (m_originalCaster)
        {
            ...
            if (m_spellAura)
            {
                ...
                else
                {
					...
					if (m_spellInfo->IsChanneled())
						m_originalCaster->ModSpellCastOrDurationTime(aurSpellInfo, duration, this, true);
...
void Spell::prepare(SpellCastTargets const* targets, AuraEffect const* triggeredByAura)
{
    ...
    if ((m_spellInfo->IsChanneled() || m_casttime) && m_caster->GetTypeId() == TYPEID_PLAYER && m_caster->isMoving() && m_spellInfo->InterruptFlags & SPELL_INTERRUPT_FLAG_MOVEMENT)
    {
        ...
        if (!(m_spellInfo->IsChanneled() && !m_casttime && (m_spellInfo->AttributesEx5 & SPELL_ATTR5_CAN_CHANNEL_WHEN_MOVING)))
        {
...
void Spell::handle_immediate()
{
    ...
    if (m_spellInfo->IsChanneled())
    {
        ...
        if (duration > 0)
        {
            ...
            // Apply haste mods
			m_caster->ModSpellCastOrDurationTime(m_spellInfo, duration, this, true);
...
void Spell::update(uint32 difftime)
{
        ...
        (m_spellInfo->Effects[0].Effect != SPELL_EFFECT_STUCK || !m_caster->HasUnitMovementFlag(MOVEMENTFLAG_FALLING_FAR))))
    {
        ...
        if (!IsNextMeleeSwingSpell() && !IsAutoRepeat() && !IsTriggered() && !(IsChannelActive() && (m_spellInfo->AttributesEx5 & SPELL_ATTR5_CAN_CHANNEL_WHEN_MOVING)))
            cancel();
    }
...
void Spell::TakeRunePower(bool didHit)
{
	...
	if (runeCost[RUNE_DEATH] > 0) //죽음의 룬
    {
        for (uint32 i = 0; i < MAX_RUNES; ++i)
        {
            RuneType rune = player->GetCurrentRune(i);
...
void Spell::HandleThreatSpells()
{
    ...
    else if (!(m_spellInfo->AttributesCu & SPELL_ATTR0_CU_NO_INITIAL_THREAT))
...
SpellCastResult Spell::CheckCast(bool strict)
{
    ...
    if (m_caster->GetTypeId() == TYPEID_PLAYER && !(m_spellInfo->Attributes & SPELL_ATTR0_PASSIVE))
    {
        ...
        if (m_caster->ToPlayer()->HasSpellCooldown(m_spellInfo->Id))
        {
			...
			else if (m_spellInfo->Attributes & SPELL_ATTR0_DISABLED_WHILE_ACTIVE)  // Shadowform, Tricks of the Trade, Misdirection 속임수거래
	...
	{
        if (m_spellInfo->Attributes & SPELL_ATTR0_DISABLED_WHILE_ACTIVE)   // Shadowform, Tricks of the Trade, Misdirection 속임수거래
            return SPELL_FAILED_FIZZLE;
        else
            return SPELL_FAILED_NOT_READY;
        // Return spell fizzle for shadowform, returning not ready causes it to bug out client side.
            ...
            case SPELL_AURA_MOD_INCREASE_MOUNTED_FLIGHT_SPEED:
            {
                ...
                if (m_originalCaster && m_originalCaster->GetTypeId() == TYPEID_PLAYER && m_originalCaster->IsAlive())
                {
                    ...
                    if (AreaTableEntry const* area = GetAreaEntryByAreaID(m_originalCaster->GetAreaId()))
                        if ((area->flags & AREA_FLAG_NO_FLY_ZONE || m_originalCaster->GetZoneId() == 4197) && m_spellInfo->Id != 55173 )
