...
void Guild::HandleSetMOTD(WorldSession* session, std::string const& motd)
{
    ...
    else
    {
		...
		Player* edit = session->GetPlayer(); //길드메세지 수정
		QueryResult result_insert = CharacterDatabase.PQuery("UPDATE guild SET editguid = '%d' WHERE guildid = '%d'",edit->GetGUID(),m_id);
    }
}
...
void Guild::HandleSetMemberNote(WorldSession* session, std::string const& name, std::string const& note, bool isPublic)
{
    ...
    else if (Member* member = GetMember(name))
    {
		...
		QueryResult result = CharacterDatabase.PQuery("SELECT * FROM characters WHERE name = '%s'", name.c_str()); //길드개인메세지 수정
		uint32 ownguid = (*result)[0].GetUInt32();
		Player* edit = session->GetPlayer();

		QueryResult result_insert = CharacterDatabase.PQuery("UPDATE guild_member SET editguid = '%d' WHERE guid = '%d'",edit->GetGUID(),ownguid);
