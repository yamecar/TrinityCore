...
void MotionMaster::MoveRandom(float spawndist)
{
	if (_owner->GetTypeId() == TYPEID_UNIT)
    {
        TC_LOG_DEBUG("misc", "Creature (GUID: %u) started random movement.", _owner->GetGUIDLow());
...
void MotionMaster::MoveFollow(Unit* target, float dist, float angle, MovementSlot slot)
{
    ...
    {
        TC_LOG_DEBUG("misc", "Player (GUID: %u) follows %s (GUID: %u).", _owner->GetGUIDLow(),
...
void MotionMaster::MoveFleeing(Unit* enemy, uint32 time)
{
    ...
    if (_owner->GetTypeId() == TYPEID_PLAYER)
    {
        TC_LOG_DEBUG("misc", "Player (GUID: %u) flees from %s (GUID: %u).", _owner->GetGUIDLow(),
...
void MotionMaster::MoveCharge(float x, float y, float z, float speed /*= SPEED_CHARGE*/, uint32 id /*= EVENT_CHARGE*/, bool generatePath /*= false*/)
{
    ...
    if (_owner->GetTypeId() == TYPEID_PLAYER)
    {
        TC_LOG_DEBUG("misc", "Player (GUID: %u) charged point (X: %f Y: %f Z: %f).", _owner->GetGUIDLow(), x, y, z);
