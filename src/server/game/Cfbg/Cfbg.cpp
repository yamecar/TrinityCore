...
void Player::FitPlayerInTeam(bool action, Battleground* pBattleGround)
{
    ...
    if (pBattleGround && action)
        SendChatMessage("%s현재 %s%s 진영으로 %s 전장에 참여하셨습니다.", MSG_COLOR_WHITE, GetTeam() == ALLIANCE ? MSG_COLOR_DARKBLUE"얼라이언스" : MSG_COLOR_RED"호드", MSG_COLOR_WHITE, pBattleGround->GetName());
}
