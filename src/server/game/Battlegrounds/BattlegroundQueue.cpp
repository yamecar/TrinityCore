            ...
            if (Battleground* bg = sBattlegroundMgr->GetBattlegroundTemplate(ginfo->BgTypeId))
            {
                if (sWorld->getBoolConfig(BATTLEGROUND_CROSSFACTION_ENABLED))
                {
                    ...
                    else
                    {
                        ...
                        ss << "|cffff0000[크로스펙션]:|r " << bgName << " -- [" << q_min_level << "-" << q_max_level << "] " << qPlayers << "/" << MinPlayers;
...
void BattlegroundQueue::FillPlayersToBG(Battleground* bg, BattlegroundBracketId bracket_id)
{
	...
	if (sWorld->getIntConfig(CONFIG_BATTLEGROUND_MATCH_TYPE) == 1)
		uint32 matchCount = 1;
    ...
    */
    ...
    while (abs(diffAli - diffHorde) > matchCount && (m_SelectionPools[TEAM_HORDE].GetPlayerCount() > 0 || m_SelectionPools[TEAM_ALLIANCE].GetPlayerCount() > 0))
    {
...
/*	QueryResult result_del_character_joinlsat = CharacterDatabase.PQuery("SELECT * FROM characters WHERE logout_time < UNIX_TIMESTAMP() - 31536000 and level < 56 and online = 0");
    if (result_del_character_joinlsat)
    {
         do
         {
            Field* fields = result_del_character_joinlsat->Fetch();
            Player::DeleteFromDB(fields[0].GetUInt32(), fields[1].GetUInt32(), true, true);
         }
         while (result_del_character_joinlsat->NextRow());
    }
*/
	QueryResult result_del_battle_join = CharacterDatabase.PQuery("DELETE FROM battle_join_reset WHERE reset < UNIX_TIMESTAMP() - 120");
	QueryResult result_del__emote = CharacterDatabase.PQuery("DELETE FROM emote_reset WHERE reset < UNIX_TIMESTAMP() - 15");
	...
	if (t->tm_mday == 1 && t->tm_hour >= 04)
	{
		QueryResult result_reset_on = CharacterDatabase.PQuery("SELECT * FROM arena_reset_on WHERE seasonal = 0");
		if (result_reset_on)
		{
			QueryResult result_reset_end = CharacterDatabase.PQuery("UPDATE arena_reset_on set seasonal = '1'");
			sWorld->SendWorldText(LANG_ANNOUNCE_9);
			QueryResult result1 = CharacterDatabase.PQuery("INSERT INTO arena_finish (data) values (UNIX_TIMESTAMP())");

			sArenaTeamMgr->Settlementofaccountsattheendofthemonth();
		}
	}

	
	if (t->tm_mday == 26 && t->tm_hour >= 04)
	{
		QueryResult result_reset_check = CharacterDatabase.PQuery("SELECT * FROM arena_reset_on WHERE seasonal = 1");
		if (result_reset_check)
			QueryResult result_reset_start = CharacterDatabase.PQuery("UPDATE arena_reset_on set seasonal = '0'");
	}
}
