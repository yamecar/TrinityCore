...
void ArenaTeamMgr::Settlementofaccountsattheendofthemonth()
{
	QueryResult result_gladiator_titles3 = CharacterDatabase.PQuery("SELECT * FROM arena_team_member WHERE personalRating >= 2000 and personalRating < 2300");
	if(result_gladiator_titles3)
	{
		do
		{
			uint32 captianguid = (*result_gladiator_titles3)[1].GetUInt32();
			std::string text = "격노한 검투사 칭호를 획득하였습니다.";
			Player* receiver = ObjectAccessor::FindPlayer(captianguid);

			MailSender sender(MAIL_NORMAL, 2, MAIL_STATIONERY_GM);

			// fill mail
			MailDraft draft("분기 투기장 보상", text);

			SQLTransaction trans = CharacterDatabase.BeginTransaction();

			if (Item* item = Item::CreateItem(968, 1))
			{
				item->SaveToDB(trans);                               // save for prevent lost at next mail load, if send fail then item will deleted
				draft.AddItem(item);
			}

			draft.SendMailTo(trans, MailReceiver(receiver, GUID_LOPART(captianguid)), sender);
			CharacterDatabase.CommitTransaction(trans);
		}
		while (result_gladiator_titles3->NextRow());
	}

	QueryResult result_gladiator_titles2 = CharacterDatabase.PQuery("SELECT * FROM arena_team_member WHERE personalRating >= 2300 and personalRating < 2600");
	if(result_gladiator_titles2)
	{
		do
		{
			uint32 captianguid = (*result_gladiator_titles2)[1].GetUInt32();
			std::string text = "냉혹한 검투사 칭호를 획득하였습니다.";
			Player* receiver = ObjectAccessor::FindPlayer(captianguid);
			...
			if (Item* item = Item::CreateItem(967, 1))
			{
				item->SaveToDB(trans);                               // save for prevent lost at next mail load, if send fail then item will deleted
				draft.AddItem(item);
			}
		...
		while (result_gladiator_titles2->NextRow());
	}

	QueryResult result_gladiator_titles1 = CharacterDatabase.PQuery("SELECT * FROM arena_team_member WHERE personalRating >= 2600");
	if(result_gladiator_titles1)
	{
		do
		{
			uint32 captianguid = (*result_gladiator_titles1)[1].GetUInt32();
			std::string text = "분노한 검투사 칭호를 획득하였습니다.";
			Player* receiver = ObjectAccessor::FindPlayer(captianguid);
			...
			if (Item* item = Item::CreateItem(966, 1))
			{
				item->SaveToDB(trans);                               // save for prevent lost at next mail load, if send fail then item will deleted
				draft.AddItem(item);
			}
		...
		while (result_gladiator_titles1->NextRow());
	}

	QueryResult result_arena = CharacterDatabase.PQuery("SELECT * FROM arena_team");
	if(result_arena)
	{
		do
		{
			uint32 teamId = (*result_arena)[0].GetUInt32();
			ArenaTeam* arena = sArenaTeamMgr->GetArenaTeamById(teamId);
			arena->Disband();
			delete(arena);
		}
		while (result_arena->NextRow());
	} 
}
