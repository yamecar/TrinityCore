...
void BattlegroundWS::PostUpdateImpl(uint32 diff)
{
    if (GetStatus() == STATUS_IN_PROGRESS)
    {
        ...
        {
            ++_minutesElapsed;
            UpdateWorldState(BG_WS_STATE_TIMER, 15 - _minutesElapsed);
        }
        ...
        if (_bothFlagsKept)
        {
            ...
            if (_flagDebuffState == 0 && _flagSpellForceTimer >= 3*MINUTE*IN_MILLISECONDS)  //10 minutes  노래방디법
            {
            ...
            else if (_flagDebuffState == 1 && _flagSpellForceTimer >= 5*MINUTE*IN_MILLISECONDS) //15 minutes
            {
                if (Player* player = ObjectAccessor::FindPlayer(m_FlagKeepers[0]))
                {
...
void BattlegroundWS::StartingEventCloseDoors()
{
    ...
    UpdateWorldState(BG_WS_STATE_TIMER, 15);
}
...
void BattlegroundWS::FillInitialWorldStates(WorldPacket& data)
{
    ...
    if (GetStatus() == STATUS_IN_PROGRESS)
    {
        ...
        data << uint32(BG_WS_STATE_TIMER) << uint32(15-_minutesElapsed);
    }
    else
