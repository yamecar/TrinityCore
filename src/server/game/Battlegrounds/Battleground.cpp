...
void Battleground::EndBattleground(uint32 winner)
{
            ...
            if (winner != WINNER_NONE)
            {
                ...
                if (sWorld->getBoolConfig(CONFIG_ARENA_LOG_EXTENDED_INFO))
                    for (auto const& score : PlayerScores)
                        if (Player* player = ObjectAccessor::FindConnectedPlayer(MAKE_NEW_GUID(score.first, 0, HIGHGUID_PLAYER)))
                        {
    ...
    for (BattlegroundPlayerMap::iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
    {
        ...
        if (team == winner)
        {
			...
			{
				// 전장 보상
				player->AddItem(16086,10);   //승리자의 문장
				player->AddItem(7168,5);    //빛의 문장
				player->AddItem(4573,2);    //보급 가방
				player->AddItem(20559,5);    //아라시 문장
			}
        ...
        else
        {
			...
			if(!isArena())
			{
				player->AddItem(16086,5);   //승리자의 문장
				player->AddItem(7168,2);    //빛의 문장
				player->AddItem(4573,1);    //보급 가방
				player->AddItem(20559,3);    //아라시 문장
			}
        }
		...
		if(isArena())  //투기장 종료 아이피삭제
		{
			uint32 arena_team_id = player->GetArenaTeamId(0);
...
{
	player->RemoveAurasDueToSpell(11593);
